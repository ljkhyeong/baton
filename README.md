...
진행 중인 시즌에서 변경 권한이 있고 변경사항 전달이 끝나면 ‘이번 주 요약 만들기’를 사용할 수 있다. 내용이 같으면 기존 요약을 불러오며, 종료 시즌에서는 저장된 요약만 조회한다. ‘요약 공유’, ‘요약 링크 복사’와 ‘인쇄·PDF 저장’도 제공한다. 공유 링크를 받은 사람에게도 로그인과 팀 접근 권한이 필요하다.
...
[서비스 상태 페이지](docs/runbooks/free-integrations.md#사용자용-서비스-상태-페이지)는 Better Stack 무료 페이지를 `status.b4ton.com`에 연결한다. 공개 확인 후 `BATON_STATUS_PAGE_ENABLED=true`로 웹 이미지를 다시 빌드하면 시작·로그인·오류 화면에 `서비스 상태` 링크가 나타난다. 기본값은 숨김이며 실제 페이지 생성·DNS 연결은 남아 있다.

[감시 시스템 하트비트](docs/runbooks/free-integrations.md#감시-시스템-하트비트)는 Prometheus·Alertmanager가 계속 동작하는지 Better Stack에서 확인하는 선택 설정이다. 감시 경로가 보내는 5분 주기 신호가 끊기면 외부 이메일로 알린다. 백업 하트비트와 별도 무료 슬롯·URL을 사용한다.
