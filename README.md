...
- application 경계의 session 구성원·레거시 공유 키 분리 검증, 원문 키 비저장과 구성원·역할·루틴 정의·시즌 회차·회차 실행·역할 자료·결정·바통 항목·역할 바통의 겹친 수정 충돌 처리
...
생명주기와 workspace API의 세부 역할별 권한은 아직 구현하지 않았다. ROUND 참여권은
활성 구성원에게만 `participant`로 발급하며, 역할 담당자와 `host` 권한의 의미는 후속
계약으로 남긴다. 일반 workspace API는 access-key header가 없으면 active session
membership, header가 있으면 레거시 공유 키를 검증하며 한 방식의 실패를 다른 방식으로
fallback하지 않는다. session 변경은 CSRF가 필수이고 익명 레거시 요청만 과도기
예외다. 첫 파일럿 배포는 Docker Compose와 Caddy를
...
4. 아직 account에 결속되지 않은 레거시 사용자의 fragment 접근 키는 읽기·쓰기
   권한과 같으므로 공개 채널에 게시하거나 브라우저 영속 저장소에 보관하지 않는다.
...
사용자는 홈에서 Google OIDC 로그인을 완료하고 받은 token을 먼저 미리 본 뒤 명시적으로
수락한다. 작업 공간의 `계정·초대` 화면은 공유 키가 아니라 session account의 활성
membership과 `OWNER` 역할을 확인한 뒤 일반 구성원 초대를 발급·조회·폐기한다. 발급 token은
현재 화면에서 안전하게 전달한 뒤 지우며, 브라우저는 응답 유실 복구용 UUID와 대상만
저장한다. 실제 파일럿 사용자를 전환하기 전에는 실제 Google redirect·cookie·CSRF와 서로
다른 두 계정의 OWNER 발급·MEMBER 수락을 공개 HTTPS 브라우저에서 검증해야 한다.
