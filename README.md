...
- `오늘`: 재확인 기한이 된 자료·현재 담당자 목록, 관련 운영 회차의 예정·진행·지연·완료 반복 업무와 운영 점검, 계정에 연결한 구성원의 미완료 담당 반복 업무·수락 대기 인수인계, BRIEF 주간 운영 요약 조회·생성
...
`주간 운영 요약`은 오늘 화면에서 펼칠 때 조회한다. 로그인 계정과 활동 중인 팀 구성원 연결,
팀 접근 권한이 필요하며 BRIEF 서비스 연결도 설정되어 있어야 한다. 공유 키 팀은 접근 키를 함께 사용한다. 생성 당시의 주차·시각·운영
...
제품 데이터는 MySQL에 저장한다. React Query는 팀·시즌의 조회 데이터인 워크스페이스
프로젝션을 불러온다. 구성원, 역할, 루틴·회차, 결정, 자료, 바통 이력과 운영 주의사항을 포함한다.

| 항목 | 동작 |
| --- | --- |
| 화면 갱신 | 열린 화면에서 10초마다 조회. 탭 복귀·재연결 시 즉시 조회하며 수동 새로고침도 제공 |
| 조회 실패 | 일시적인 실패에는 기존 내용 유지. 폐기된 접근 키의 `403`은 접근 오류 화면으로 전환 |
| 오류 안내 | 연결 실패·응답 지연·잘못된 응답을 구분해 재시도 안내. 서버 5xx에는 요청 ID 표시 |
| 운영 점검 | 담당자·후임 누락, 준비 부족, 반복 지연, 인수인계 미완료와 담당 기간 공백의 이유·다음 행동 표시 |
| 기록 검색 | 선택한 시즌 또는 팀의 모든 시즌에서 결정·인수인계 항목·자료 검색. 별도 검색 API 없이 시즌별 조회 데이터 사용 |
| 보관·종료 | 보관 기록과 종료 시즌도 조회 가능. 반복 업무 보관 뒤에도 과거 회차의 실행 기록과 완료 처리 유지. 종료 시즌의 일반 콘텐츠 수정은 차단 |

데이터 범위와 상태별 동작은 [제품 기준선](docs/PRD/0001_product-baseline/spec.md#7-현재-구현-상태),
요청·오류·트랜잭션 시간 제한은 [API 계약](docs/PRD/0002_api-contract/spec.md)을 따른다.
트랜잭션은 함께 성공하거나 취소되는 DB 작업 단위다.

#### 시간과 접근 권한

날짜·시즌 진행률·종료 안내·마감은 브라우저 위치가 아닌 시즌의 IANA 시간대를 기준으로 한다.
기존·최초 시즌은 `Asia/Seoul`을 사용한다. 다음 시즌은 시간대를 이어받고 회차 일정은 새로 설정한다.

공유 키는 파일럿 작업 공간의 접근 권한에 사용한다. 계정 세션은 로그인 신원, 기존 구성원 연결과
ROUND 참여권에 사용한다. 잘못된 새 링크는 기존 정상 접근 키를 덮어쓰지 않는다.
운영자가 팀별로 계정 권한을 켜면 관리자·구성원·열람자 권한과 초대로 접근하고 기존 공유 키를 거부한다.
공유 키 팀의 인수인계 확인은 명의 선언으로 유지하며, 계정 팀은 로그인한 구성원과 확인자를 대조한다.
자세한 기준은 [제품 기준선](docs/PRD/0001_product-baseline/spec.md)과
[시즌 종료 결정](docs/ADR/0011_season_lifecycle/adr.md)을 따른다.

#### 저장 오류와 중복 요청 복구

멱등성은 같은 요청을 반복해도 중복 처리하지 않는 성질이다. 브라우저에는 팀별 공유 키,
최근 작업 공간 정보와 응답 유실 복구에 필요한 입력·멱등 키를 보관한다.

- 작업 공간·콘텐츠·다음 시즌 생성과 접근 키 변경은 복구 정보를 저장한 뒤에만 전송한다.
- 같은 브라우저의 여러 탭에서 동시에 요청하면 먼저 시작한 요청의 결과를 확인한다.
- 완료 여부가 불명확하면 저장한 입력과 같은 키로 재확인한다. 새 요청으로 자동 전환하지 않는다.
- 콘텐츠 생성·접근 키 변경 성공 또는 최종 실패 후 복구 기록을 정리한다. 삭제에 실패하면 정리한 뒤 새 요청을 시작한다.
- 접근 키 변경은 결과 확인·정리까지 탭 간 잠금을 유지한다. 잠금을 지원하지 않으면 변경을 시작하지 않는다.

재시도·복구 기록 만료·다른 탭과의 충돌·수동 폐기 조건은
[제품 기준선](docs/PRD/0001_product-baseline/spec.md#7-현재-구현-상태)과
[API 계약](docs/PRD/0002_api-contract/spec.md)을 따른다.
...
열린 시즌에서 변경 권한이 있고 변경사항 전송이 끝나면 이번 주 브리프를 생성할 수 있다. 생성 요청은 자동으로
...
- 현재 역할·회차·인수인계 기록에서 이유와 다음 행동을 계산하는 운영 점검 프로젝션
...
- `BATON WATCH`: 역할 자료 URL 스냅샷의 비동기 상태 점검, SSRF 방어, 임대·시도·결과·현재 건강 상태와 상태 변경 이벤트 전달을 소유한다. BATON은 감시 적격 자료 변경과 시즌 생명주기를 불변 트랜잭셔널 아웃박스에 기록하고 기능을 활성화한 뒤 커밋 이후 WATCH 모니터로 전달·재조정한다. WATCH가 최소 한 번 전달 방식으로 보낸 이벤트는 별도 인증의 트랜잭셔널 인박스에 원자적으로 수신하지만, 역할 상세의 별도 현재 상태 조회·재점검 UI도 제공한다. 실제 공개 스테이징의 WATCH→BATON 전달·동일 재전송과 운영 활성화는 아직 검증하지 않았다.
