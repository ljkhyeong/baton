...
BATON은 팀의 역할, 반복 업무, 결정 기록과 인수인계를 관리하는 서비스다.
...
- `오늘`: 관련 운영 회차의 예정·진행·지연·완료 루틴과 운영 주의사항, 계정에 연결한 구성원의 미완료 담당 루틴·수락 대기 바통, BRIEF 주간 운영 요약 조회·생성
- `역할`: 현재 팀의 구성원 추가·이름 정정·활동 종료·재활성화, 로그인 계정과 기존 구성원 연결, 현재 시즌 역할의 목적, 책임, 현재·다음 담당자와 담당 기간 등록·수정, 참고 자료 링크 연결
- `운영`: 모임 전·중·후 반복 루틴과 실제 마감 정의, 사용하지 않는 루틴 정의의 가역 보관·복원, 주간·격주 자동 일정, 수동 회차 생성, 수동·자동 회차의 이름·날짜 정정, 이번 자동 회차 건너뛰기와 보관·복원과 회차별 완료 처리
...
공유 접근 키가 필요하며 BRIEF 서비스 연결도 설정되어 있어야 한다. 생성 당시의 주차·시각·운영
...
| 운영 주의사항 | 담당자·후임 누락, 준비 부족, 반복 지연, 인수인계 미완료와 담당 기간 공백의 이유·다음 행동 표시 |
| 기록·자료 검색 | 선택한 시즌의 결정·바통 항목·자료 검색. 별도 검색 API·캐시 없이 이미 조회한 데이터 사용 |
| 보관·종료 | 보관 기록과 종료 시즌도 조회 가능. 루틴 보관 뒤에도 과거 회차의 실행 기록과 완료 처리 유지. 종료 시즌의 일반 콘텐츠 수정은 차단 |
...
공유 키를 대체할 초대·세부 권한은 미결정이다. 기존 역할 바통의 구성원 확인은 공유 키 안에서
이름을 선택하는 방식이므로 실제 로그인 신원을 인증한 감사 기록으로 볼 수 없다.
...
- 계정 보안 설정: `/account`에서 연결된 Google·Naver·자체 이메일 로그인 수단을 확인하고, 자체 이메일 비밀번호 변경과 모든 기기 로그아웃을 수행한다. 두 작업은 현재 세션까지 즉시 종료하지만 공유 접근 키와 이미 발급된 ROUND 참여권은 유지한다.
...
- 현재 역할·회차·바통 기록에서 이유와 다음 행동을 계산하는 운영 주의사항 프로젝션
...
2. 생성된 작업 공간의 역할 화면에서 구성원을 추가하고 이름·활동 상태를 관리한다. 활동 종료 구성원은 기존 역할·결정에 남지만 새 담당자와 작성자 선택에서는 제외된다. 역할과 반복 루틴·실제 마감을 등록·수정하며, 역할 상세에 함께 사용할 문서 링크를 연결한다. 오늘 화면의 운영 주의사항에서 담당자·후임 공백, 준비 부족과 반복 지연의 이유와 다음 행동을 확인한다. 운영 화면에서 시즌 시간대와 주간·격주 일정을 켜 회차를 미리 자동 생성하거나 필요할 때 수동 회차를 만든다. 수동·자동 회차는 이름·날짜를 정정하고 모든 활성 회차의 루틴 실행을 완료 처리한다. 더 이상 반복하지 않는 루틴 정의는 보관함으로 옮겨 새 수동·자동 회차와 다음 시즌에서 제외하되, 보관 전 회차의 실행은 계속 완료 처리한다. 정의를 복원하면 다음 회차부터 다시 포함된다. 모든 정의를 보관한 동안 자동 일정은 빈 회차를 만들지 않고 발생 커서를 전진시키며 화면은 `활성 루틴 대기 중`으로 안내한다. 현재 운영에서 치울 회차·결정·바통 항목은 보관했다가 필요할 때 기존 실행 기록과 상태 그대로 복원한다. 탐색 화면에서는 결정의 이유와 관련 역할, 바통 항목과 자료를 검색하고 역할·상태·시즌 시간대 기준 기간으로 좁힌다. 역할을 교대할 때는 바통 화면에서 다음 담당자와 담당 기간을 정해 준비하고, 누락 경고를 확인해 전달한 뒤 다음 담당자 명의로 수락한다. 시즌이 끝나면 열린 역할 바통을 먼저 수락하거나 취소하고 상단 시즌 전환에서 다음 시즌을 시작해 이어 갈 역할과 활성 루틴만 고른다. 담당자·담당 기간, 회차 일정과 실행 상태는 새 시즌에서 다시 정하며, 과거 시즌은 읽기 전용으로 계속 조회하고 해당 시즌 안에서 탐색한다.
...
- 계정 초대·탈퇴, 추가 인증 기반 신원 연결·병합과 관리자에 의한 세션 강제 만료
