...
스터디 예시 데이터를 이용해 다음 흐름을 탐색할 수 있다.

- `오늘`: 이번 회차의 역할별 운영과 지연 확인
- `역할`: 역할의 목적, 책임, 현재·다음 담당자 확인
- `운영`: 모임 전·중·후 반복 루틴 확인과 완료 처리
- `기록`: 결정 내용, 이유, 대안과 관련 역할 기록
- `바통`: 역할별 인수인계 항목과 바통북 미리보기

프로토타입 상태는 브라우저 `localStorage`에 저장한다. 실제 계정, 서버 저장과 다중 팀 동기화는 아직 연결하지 않았다.
...
라우트와 QueryClient, 공용 API client와 오류 모델은 준비되어 있다. 현재 제품 데이터는 `localStorage` 프로토타입을 사용하며 백엔드 제품 API와는 아직 연결하지 않았다.
