...
- `오늘`: 관련 운영 회차의 예정·진행·지연·완료 루틴과 조직 연속성 레이더, 계정에 연결한 구성원의 미완료 담당 루틴·수락 대기 바통
...
저장된 브리프는 이번 주 변경·이전부터 미해소를 구분한다. 이전 항목은 분류 미기록으로
표시하며 상태 전이 상세에는 저장된 원본 심각도(긴급·주의·미기록)를 함께 표시한다.

‘오늘’ 화면의 `BRIEF 관심 항목`을 열면 활성 심각도·공백 요약, 이번 주 해소 건수와 상태별 필터 목록을 조회한다.
해소 건수는 실제 활성·해소 전환을 확인했고 현재도 해소된 항목만 센다. 건수를 누르면 해당
업무와 해소 시점을 보고 현재 업무로 이동할 수 있다. 생성·재사용이 성공하면 필터는 유지하고
관심 목록·해소 목록을 첫 페이지부터 다시 불러온다. 저장된 브리프의
‘지난주와 바로 비교’는 같은 시간대의 지난주 마지막 브리프를 찾아 연결한다.
...
전달 대기·실패와 생성 준비 상태를 확인한다. 저장 이후 추가 전달 여부는 마지막 성공
생성·재사용 기록을 기준으로 별도 안내하며 확인 근거가 없으면 알 수 없다고 표시한다.
항목별 담당자·루틴·인수인계 확인 버튼을 제공하고 원본 참조·리비전은 펼쳐 보는 기록으로
표시한다. 영역을 접었다 펼치거나 업무 화면에서 ‘오늘’로 돌아와도 필터·조회 생성본·비교
선택을 유지한다. 새로고침에는 링크에 지정한 생성본만 복원한다. ‘이 브리프 링크 복사’로
특정 생성본을 공유할 수 있으며 링크를 받은 사람도 기존 로그인·팀 접근 권한이 필요하다.
‘이 브리프 인쇄·PDF 저장’은 선택한 생성 당시 항목을 출력하고 현재 업무명은 별도로 구분한다.
인쇄 버튼은 주소의 접근 키를 잠시 숨기고 인쇄가 끝나면 복원한다. 상세 계약은
