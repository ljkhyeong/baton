...
결정·자료·인수인계 항목 작성 창은 확정 전 본문을 현재 탭에 자동 저장한다. 같은 계정·팀·시즌·접근 키·기록으로 다시 열면 24시간 이내의 초안을 직접 불러오거나 삭제할 수 있다. 작성자와 연결 역할은 현재 선택을 유지하므로 저장 전에 확인한다. 서버 저장 성공 시 초안을 지우며, 저장소 실패는 화면에 알린다. 다른 기기와 동기화하지 않는다.
