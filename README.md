...
오늘 화면의 내 알림함은 로그인 계정과 연결된 활성 구성원의 현재 역할을 기준으로 24시간 이내 마감·지연 업무와 수락 대기 바통을 표시한다. 읽음은 계정별로 서버에 저장하며 마감 임박과 지연은 별도 알림이다. 마감 시각을 바꾸면 새 알림으로 보인다. 완료·회차 보관·바통 수락·취소·시즌 종료 뒤에는 해당 알림이 현재 목록에서 빠진다. 원본 회차·바통으로 이동할 수 있고 열린 화면은 30초마다 갱신한다. 이메일·외부 메시지를 전송하거나 과거 알림 전체를 보관하는 기능은 포함하지 않는다.
