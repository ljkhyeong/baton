...
첫 화면에서 팀, 시즌 기간과 구성원을 등록하면 공유 가능한 스터디 작업 공간을 만든다. 빈 구성 또는 스터디·팀 운영 시작 템플릿을 선택할 수 있으며 템플릿은 담당자가 비어 있는 역할 3개와 루틴 3개를 함께 만든다.

- `내 팀`: 로그인 계정이 승인된 팀 목록·현재 권한 조회와 여러 팀 전환. 진행 중인 모든 시즌의 미완료 담당 루틴·수락 대기 바통 모아 보기와 원본 이동. 시작 화면·작업 공간에서 `/my-teams`로 이동
- `오늘`: 재확인 기한이 된 자료·현재 담당자 목록, 관련 운영 회차의 예정·진행·지연·완료 루틴과 확인이 필요한 업무, 계정에 연결한 구성원의 미완료 담당 루틴·수락 대기 바통, BRIEF 주간 운영 요약 조회·생성
...
- `기록`: 결정 내용, 이유, 대안과 관련 역할 등록·수정, 이유·대안의 Markdown 편집·미리보기, 가역 보관·복원과 보관함
- `바통`: 역할별 인수인계 항목 등록·수정·완료와 가역 보관·복원, 완료 항목·역할 책임·주의사항을 포함한 인수인계 문서 미리보기와 인쇄·PDF 저장, 다음 담당 기간을 정한 바통 준비·전달·수락·취소
- `탐색`: 현재 시즌 또는 팀의 모든 시즌에 걸친 결정·바통 항목·역할 자료 통합 검색, 종류·역할·활성/보관 상태·기간 필터와 원본 화면 이동
...
오늘 화면의 내 알림함은 로그인 계정과 연결된 활성 구성원의 현재 역할을 기준으로 마감 임박·지연 업무와 수락 대기 바통을 표시한다. 기본 사전 알림은 24시간이며 계정별로 1~168시간과 세 가지 알림의 표시 여부를 저장한다. 설정은 모든 팀·기기에 적용하며 껐다 다시 켜도 기존 읽음 표시는 유지한다. 읽음은 계정별로 서버에 저장하며 마감 임박과 지연은 별도 알림이다. 마감 시각을 바꾸면 새 알림으로 보인다. 완료·회차 보관·바통 수락·취소·시즌 종료 뒤에는 해당 알림이 현재 목록에서 빠진다. 원본 회차·바통으로 이동할 수 있고 열린 화면은 30초마다 갱신한다. 이메일·외부 메시지를 전송하거나 과거 알림 전체를 보관하는 기능은 포함하지 않는다.
...
다음 시즌으로 이어 온 역할은 상세에서 이전 시즌의 목적·책임, 자료, 결정 이유와 바통 항목을 펼쳐 볼 수 있다. 필요한 자료만 현재 역할의 추가 양식에 가져와 확인한 뒤 새로 저장하며, 원본 시즌의 기록은 유지한다. 이전 시즌 연결이 없는 기존 이관 역할에는 이 기능을 표시하지 않는다.

역할 자료는 링크와 설명을 보존한 채 보관·복원할 수 있다. 보관한 자료는 일반 역할 화면, 바통 준비 자료 수, ROUND 방 동작과 WATCH 활성 감시 대상에서 제외하고 보관함과 탐색 기록에는 남긴다.

결정·자료·바통 항목 작성 창은 확정 전 본문을 현재 탭에 자동 저장한다. 같은 계정·팀·시즌·접근 키·기록으로 다시 열면 24시간 이내의 초안을 직접 불러오거나 삭제할 수 있다. 작성자와 연결 역할은 현재 선택을 유지하므로 저장 전에 확인한다. 서버 저장 성공 시 초안을 지우며, 저장소 실패는 화면에 알린다. 다른 기기와 동기화하지 않는다.
...
- 내 계정: `/account`에서 내 캘린더 구독 목록·상태 확인·해제와 계정 보안 설정을 제공한다. 연결된 Google·Naver·자체 이메일 로그인 수단을 확인하고, 자체 이메일 비밀번호 변경과 모든 기기 로그아웃을 수행한다. 두 작업은 현재 세션까지 즉시 종료하지만 공유 접근 키와 이미 발급된 ROUND 참여권은 유지한다.
...
- 현재 역할·회차·바통 기록에서 이유와 다음 행동을 계산하는 확인이 필요한 업무 프로젝션
...
2. 생성된 작업 공간의 역할 화면에서 구성원을 추가하고 이름·활동 상태를 관리한다. 활동 종료 구성원은 기존 역할·결정에 남지만 새 담당자와 작성자 선택에서는 제외된다. 역할과 반복 루틴·실제 마감을 등록·수정하며, 역할 상세에 함께 사용할 문서 링크를 연결한다. 오늘 화면의 확인이 필요한 업무에서 담당자·후임 공백, 준비 부족과 반복 지연의 이유와 다음 행동을 확인한다. 운영 화면에서 시즌 시간대와 주간·격주 일정을 켜 회차를 미리 자동 생성하거나 필요할 때 수동 회차를 만든다. 수동·자동 회차는 이름·날짜를 정정하고 모든 활성 회차의 루틴 실행을 완료 처리한다. 더 이상 반복하지 않는 루틴 정의는 보관함으로 옮겨 새 수동·자동 회차와 다음 시즌에서 제외하되, 보관 전 회차의 실행은 계속 완료 처리한다. 정의를 복원하면 다음 회차부터 다시 포함된다. 모든 정의를 보관한 동안 자동 일정은 빈 회차를 만들지 않고 발생 커서를 전진시키며 화면은 `활성 루틴 대기 중`으로 안내한다. 현재 운영에서 치울 회차·결정·바통 항목은 보관했다가 필요할 때 기존 실행 기록과 상태 그대로 복원한다. 탐색 화면에서는 결정의 이유와 관련 역할, 바통 항목과 자료를 검색하고 역할·상태·시즌 시간대 기준 기간으로 좁힌다. 역할을 교대할 때는 바통 화면에서 다음 담당자와 담당 기간을 정해 준비하고, 누락 경고를 확인해 전달한 뒤 다음 담당자 명의로 수락한다. 시즌이 끝나면 열린 역할 바통을 먼저 수락하거나 취소하고 상단 시즌 전환에서 다음 시즌을 시작해 이어 갈 역할과 활성 루틴만 고른다. 담당자·담당 기간, 회차 일정과 실행 상태는 새 시즌에서 다시 정하며, 과거 시즌은 읽기 전용으로 계속 조회하며 모든 시즌 검색에서 함께 탐색할 수 있다.
...
- `e2e:handoff`: 역할 자료 생성의 응답 유실 복구, 수정 충돌 최신화, 보관·복원, 새 창 열기·재조회, 바통 항목·인수인계 문서 미리보기와 역할 바통 준비·경고 확인·전달·수락·새로고침 보존 흐름
