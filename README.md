...
지정한다. 스케줄러 시간 대기, 복원 재전달과 실제 캘린더 앱 검증은 포함하지 않는다.
...
- CAL 시즌 이름: `BATON_CAL_SEASON_METADATA_ENABLED=false`가 기본값이다. 검증 환경에서 이름
  ...
  적체와 실패를 확인한다. 기존 시즌 보정·복원 재전달·후보 계약 채택을 마치기 전 운영 활성화는
  보류한다. 자세한 설정과 CAL V7 배포 조건은 [PRD-0006](docs/PRD/0006_calendar-integration-contract/spec.md)을 따른다.
