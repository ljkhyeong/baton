...
첫 화면에서 팀, 시즌 기간과 구성원을 등록하면 공유 가능한 스터디 작업 공간을 만든다. 빈 구성 또는 스터디·팀 운영 시작 템플릿을 선택할 수 있으며 템플릿은 담당자가 비어 있는 역할 3개와 반복 업무 3개를 함께 만든다.

- `내 팀`: 로그인 계정이 승인된 팀 목록·현재 권한 조회와 여러 팀 전환. 진행 중인 모든 시즌의 미완료 담당 반복 업무·수락 대기 인수인계 모아 보기와 원본 이동. 시작 화면·작업 공간에서 `/my-teams`로 이동
- `오늘`: 재확인 기한이 된 자료·현재 담당자 목록, 관련 운영 회차의 예정·진행·지연·완료 반복 업무와 운영 점검, 계정에 연결한 구성원의 미완료 담당 반복 업무·수락 대기 인수인계, 주간 요약 조회·생성
- `역할`: 현재 팀의 구성원 추가·이름 정정·활동 종료·재활성화, 로그인 계정과 기존 구성원 연결, 현재 시즌 역할의 목적, 담당 업무, 현재·다음 담당자와 담당 기간 등록·수정, 참고 자료 링크 연결과 이전 시즌 기록 조회·자료 이어받기
- `일정`: 모임 전·중·후 반복 업무와 실제 마감 정의, 사용하지 않는 반복 업무 정의의 보관·복원, 주간·격주 자동 일정, 수동 회차 생성, 수동·자동 회차의 이름·날짜 정정, 이번 자동 회차 건너뛰기와 보관·복원과 회차별 완료 처리
- `기록`: 결정 내용, 이유, 대안과 관련 역할 등록·수정, 이유·대안의 Markdown 편집·미리보기, 보관·복원과 보관함
- `인수인계`: 역할별 인수인계 항목 등록·수정·완료와 보관·복원, 완료 항목·담당 업무·주의사항을 포함한 인수인계 문서 미리보기와 인쇄·PDF 저장, 다음 담당 기간을 정한 인수인계 준비·전달·수락·취소
- `검색`: 현재 시즌 또는 팀의 모든 시즌에 걸친 결정·인수인계 항목·역할 자료 통합 검색, 종류·역할·활성/보관 상태·기간 필터와 원본 화면 이동
- `시즌`: 팀의 과거·현재 시즌 전환, 이름·기간 수정, 명시적 종료와 선택한 역할·반복 업무로 다음 시즌 시작

역할 자료 상세에서 사람이 확인한 최근 20건을 조회하고, 로그인 계정과 연결된 활성 구성원이 사용 가능·수정 필요와 500자 이내 메모를 남길 수 있다. 서버는 확인 당시 구성원 이름·자료 주소·자료 버전·시각을 보존한다. 자료를 수정하거나 보관·복원해 버전이 바뀌면 이전 확인으로 표시하며 새 확인이 필요하다. 종료 시즌·보관 자료에는 확인을 추가할 수 없다. 자료별로 1~365일의 재확인 주기와 첫 확인일을 설정할 수 있으며, 사용 가능으로 확인하면 시즌 날짜 기준으로 다음 확인일을 갱신한다. 주기 설정은 V37로 자료 버전과 분리해 저장하며 기본으로 꺼져 있다. WATCH 자동 점검 상태를 뜻하지 않으며 공유 키 팀의 계정·구성원 연결은 키 소유자가 직접 선택하며 계정 전환 팀은 관리자의 승인이 필요하다.
...
`주간 업무 점검`을 펼치면 현재 점검 항목을 조회하고, `저장된 주간 요약`에서 생성 당시 내용을 확인한다. 로그인 계정과 활동 중인 팀 구성원 연결,
팀 접근 권한이 필요하며 BRIEF 서비스 연결도 설정되어 있어야 한다. 공유 키 팀은 접근 키를 함께 사용한다. 생성 당시의 주차·시각·점검
항목을 표시하고, 종료 시즌은 기존 요약만 조회한다. 실제 원격 HTTPS 연동 검증과 서비스 활성화는
별도 운영 작업으로 남아 있다.
...
인수인계 문서는 미리보기의 `인쇄 / PDF 저장` 버튼으로 출력한다. 이 버튼은 인쇄 호출 전에 주소의 접근 키를 잠시 숨기고 인쇄 창이 닫히면 복원한다. 브라우저 메뉴나 단축키로 직접 인쇄하면 이 처리를 거치지 않으므로, 접근 키가 주소에 남아 있을 때는 반드시 인수인계 문서 버튼을 사용한다.
...
작은 보조 문구와 경고는 밝은 화면에서 읽을 수 있는 대비를 유지하고, 키보드 초점 표시는 밝은 본문과 어두운 탐색 영역 모두에서 구분된다. 보관함 요약, 인수인계 탭 패널과 시각적으로 감춘 확인란도 키보드 위치를 화면에 표시한다.

### 데이터 조회와 화면 갱신

제품 데이터는 MySQL에 저장하고 React Query로 조회한다. 작업 공간 조회 응답(프로젝션)에 다음을 함께 담는다.

- 활동 중·활동 종료 구성원과 서버에서 관리하는 시즌 목록
- 반복 업무 정의·회차·결정·인수인계 항목의 활성·보관 기록
- 역할 인수인계의 준비·전달·수락·취소 이력
- 현재 기록으로 계산한 업무·인수인계 점검 결과

반복 업무 정의를 보관해도 과거 회차는 생성 당시 실행 스냅샷을 표시하고 완료 처리할 수 있다.
검색은 별도 API 없이 시즌별 조회 데이터의 결정·인수인계 항목·역할 자료를 사용한다.
종료 시즌도 모든 시즌 검색에 포함한다.

운영 점검은 서버 `Clock`과 시즌 시간대로 담당자·후임 부재, 인수인계 준비 부족, 여러 회차의 업무 지연,
시작·전달·수락이 남은 인수인계와 담당 기간 사이 공백을 판단한다. 구성원의 활동 상태도 확인하며
이유와 다음 행동을 함께 반환한다. 화면에서는 다음 필드로 표시·수정 가능 여부를 구분한다.

| 필드 | 적용 |
| --- | --- |
| 구성원 `deactivatedAt` | 새 담당자·작성자로 선택할 수 있는지 확인 |
| 반복 업무 정의·기록 `archivedAt` | 일반 화면과 보관함 구분 |
| 시즌 `endedAt` | 종료 시즌을 조회만 가능한 상태로 표시 |

종료 시즌의 기록은 계속 조회할 수 있지만 일반 콘텐츠 변경은 서버와 UI 모두에서 막는다.
열린 작업 공간은 전경에서 10초마다 조회하고, 창 포커스·네트워크 복구 때 즉시 다시 조회한다.
마지막 갱신 시각과 수동 새로고침을 제공한다.

일시적인 조회 실패에는 기존 내용을 유지한다. 접근 키가 폐기된 `403`은 접근 오류 화면으로 전환한다.
연결 실패·응답 지연·해석할 수 없는 서버 응답은 각각 재시도를 안내하고, 5xx 오류에는 운영자에게
전달할 요청 ID를 표시한다. DB 연결 획득·행 잠금·쿼리에 적용하는 트랜잭션 제한은 프런트의 10초
요청 제한보다 짧게 둔다. 브라우저가 응답 대기를 끝낸 뒤 변경이 늦게 반영될 가능성을 줄이기 위해서다.

오늘 날짜·시즌 진행률·종료 안내·마감은 브라우저 위치와 관계없이 시즌의 IANA 시간대로 계산한다.
기존·최초 시즌의 기본값은 `Asia/Seoul`이다. 다음 시즌은 같은 시간대를 사용하되 회차 일정은 새로 설정한다.

### 브라우저 저장과 접근 권한

브라우저에는 팀별 공유 접근 키, 최근 작업 공간의 최소 정보, 요청 복구용 멱등 정보만 보관한다.
작업 공간·콘텐츠·다음 시즌 생성과 키 회전은 복구용 멱등 정보를 브라우저에 저장한 뒤에만 요청한다.
같은 브라우저의 여러 탭에서 생성하면 한 탭만 요청을 보내고, 나머지 탭에는 먼저 시작한 요청의 결과 확인을 안내한다.

시작 화면은 완료 여부가 불분명한 생성 요청을 목록으로 표시하고, 저장한 입력과 같은 멱등 키로 결과를
다시 확인한다. 기존 공유 링크로 결과를 확인한 요청은 경고 후 해당 복구 기록만 삭제할 수 있다.
공유 링크를 받은 구성원은 같은 작업 공간을 사용하며 잘못된 새 링크가 기존의 정상 접근 키를 덮어쓰지 않는다.

공유 키는 소규모 파일럿의 작업 공간 접근 권한으로 사용한다. 공급자 중립 계정과 서버 세션은 로그인 신원,
기존 구성원 연결과 ROUND 참여권을 담당한다. 팀별 계정 권한을 켜면 관리자·구성원·열람자 권한과 초대로
접근하며 기존 공유 키는 거부한다. 공유 키 팀의 인수인계는 사용자가 선택한 이름을 확인자로 기록하고,
계정 권한 팀은 로그인한 구성원과 확인자가 같은지 검증한다.

브라우저는 중복 생성을 막기 위해 요청 내용과 멱등 키를 임시 기록으로 보관한다.
오류가 나면 같은 요청을 재시도할지, 새 요청을 시작할지, 기존 결과를 먼저 확인할지 구분한다.
재시도할 수 없는 오류가 확정되거나 콘텐츠 생성·접근 키 변경에 성공하면 임시 기록을 삭제한다.
Web Storage 오류로 삭제하지 못하면 기록을 정리할 때까지 새 멱등 키로 요청할 수 없다.

온보딩 복구 기록이 다른 탭에서 바뀌거나 같은 요청의 재처리 결과가 만료되면,
기존 결과를 확인하기 전에는 새 요청을 시작하지 않는다.
접근 키 변경은 같은 팀의 여러 탭 중 한 곳에서만 진행한다. 임시 기록 생성부터
서버 결과 확인과 기록 삭제까지 잠금을 유지하며, 이 잠금을 지원하지 않는 브라우저에서는
키 변경을 시작하지 않는다.

다음 시즌으로 이어 온 역할은 상세에서 이전 시즌의 목적·책임, 자료, 결정 이유와 인수인계 항목을 펼쳐 볼 수 있다. 필요한 자료만 현재 역할의 추가 양식에 가져와 확인한 뒤 새로 저장하며, 원본 시즌의 기록은 유지한다. 이전 시즌 연결이 없는 기존 이관 역할에는 이 기능을 표시하지 않는다.

역할 자료는 링크와 설명을 보존한 채 보관·복원할 수 있다. 보관한 자료는 일반 역할 화면, 인수인계 준비 자료 수, ROUND 방 동작과 WATCH 활성 감시 대상에서 제외하고 보관함과 탐색 기록에는 남긴다.

결정·자료·인수인계 항목 작성 창은 확정 전 본문을 현재 탭에 자동 저장한다. 같은 탭에서 같은 계정·팀·시즌·접근 키·기록의 작성 창을 다시 열면 24시간 이내의 초안을 직접 불러오거나 삭제할 수 있다. 작성자와 연결 역할은 현재 선택을 유지하므로 저장 전에 확인한다. 서버 저장 성공 시 초안을 지우며, 저장소 실패는 화면에 알린다. 다른 기기와 동기화하지 않는다.

화면 제목은 ‘역할과 담당자’, ‘반복 업무 관리’, ‘결정 기록’, ‘인수인계 준비’로 표시한다.
루틴 완료 안내는 해당 회차의 업무 완료만 뜻하며 인수인계 전달과 구분한다. 구성원의 활동을
다시 시작하는 동작은 ‘활동 재개’로 표시한다. 로그인·계정 만들기·이메일 인증은 화면 제목과
입력 안내를 구분하고, 이메일 인증 완료는 서버가 확인한 뒤에만 안내한다.

### 주간 업무 점검

‘오늘’ 화면의 `주간 업무 점검`에서 미해결 항목 수와 심각도·기록 누락 이력별 목록을 본다.
`이번 주 해결`을 누르면 해당 업무와 해결 시점을 확인하고 현재 업무로 이동할 수 있다.
현재 상태가 다시 미해결이거나 해결 시점을 확인할 수 없는 항목은 해결 건수에서 제외한다.
`변경 이력 보기`에서는 원본 버전, 새로 발견한 기록 누락과 BATON 판정 심각도(긴급·주의·미기록)를 확인한다.
기록 누락 이력은 누락이 있었던 항목 수이며, 빠진 변경의 개수가 아니다.

로그인과 활동 중인 팀 구성원 연결, 팀 접근 권한이 필요하다. 공유 키 방식의 팀은 접근 키를
함께 사용한다. BATON 백엔드는 BRIEF 전용 HTTPS로 조회한다. 연동 기본값은 비활성이며 ‘BRIEF 연동이 꺼져 있습니다’로 안내한다.
...
`저장된 주간 요약`는 생성 당시 내용을 유지한다. 이번 주 변경·이전 주부터 미해결을 구분하고,
이전 데이터에 분류가 없으면 미기록으로 표시한다. 주차·생성 번호로 주간 요약을 선택해 비교하거나,
`지난주와 바로 비교`로 같은 시간대의 지난주 마지막 주간 요약을 찾는다. 현재 업무명과 이동 대상은
BATON에서 별도로 조회한다. 원본 항목 ID와 변경 번호는 펼쳐 보는 기록에 둔다.

열린 시즌에서 변경 권한이 있고 변경사항 전송이 끝나면 이번 주 요약을 생성할 수 있다. 생성 요청은 자동으로
재시도하지 않는다. 생성·재사용 성공 후 필터는 유지하고 점검 목록·해결 목록을 첫 페이지부터
다시 읽는다. 추가 전송 여부는 마지막 성공 생성·재사용 기록을 기준으로 안내하며, 확인 기록이
없으면 알 수 없다고 표시한다. 이 안내가 저장된 주간 요약의 내용 변경을 뜻하지는 않는다.

영역을 접었다 펼치거나 업무 화면에서 ‘오늘’로 돌아와도 필터·조회 주간 요약·비교 선택을 유지한다.
새로고침에는 링크에 지정한 주간 요약만 복원한다. `이 주간 요약 링크 복사`로 특정 주간 요약을 공유할 수
있으며 링크를 받은 사람도 기존 로그인·팀 접근 권한이 필요하다. `이 주간 요약 인쇄·PDF 저장`은
...
- 자체 이메일 비밀번호 재설정: 30분짜리 일회용 메일 링크, 계정 존재를 숨기는 접수 응답과 비밀번호 변경 시 기존 계정 세션 무효화. `/forgot-password`와 `/reset-password` 화면을 제공하며 가입 메일의 암호화 아웃박스·SMTP·요청률 제한을 재사용한다. 자동 로그인하지 않고 공유 키·발급된 ROUND 참여권은 유지한다.
- 내 계정: `/account`에서 내 캘린더 구독 목록·상태 확인·해제와 계정 보안 설정을 제공한다. 연결된 Google·Naver·자체 이메일 로그인 방법을 확인하고, 자체 이메일 비밀번호 변경과 모든 기기 로그아웃을 수행한다. 두 작업은 현재 세션까지 즉시 종료하지만 공유 접근 키와 이미 발급된 ROUND 참여권은 유지한다.
- 멱등한 팀·시즌·구성원 온보딩과 공유 키 발급
- 응답이 유실되어도 중복 저장 없이 재시도할 수 있는 기존 팀 구성원·역할·반복 업무·회차·결정·인수인계 항목·역할 자료 생성 API와 구성원 이름·활동 상태, 역할·반복 업무·회차·자료·결정·인수인계 항목 수정 API
- 역할과 분리된 `PREPARING → TRANSFERRED → ACCEPTED` 또는 `CANCELLED` 이력, 전달 준비도 스냅샷·경고 확인과 수락 시 역할 담당자·기간을 원자적으로 바꾸는 역할 인수인계 API
- 역할 자료·반복 업무 정의·회차·결정·인수인계 항목의 영구 삭제 없는 보관·복원 API
- 활성 반복 업무 정의와 실제 마감만 스냅샷하는 수동·자동 시즌 회차, 보관 전 실행을 유지하는 회차별 독립 실행과 예정·진행·지연·완료 상태 API
- 현재 역할·회차·인수인계 기록으로 점검 결과와 다음 행동을 계산하는 조회 기능
- 시즌 IANA 시간대, 주간·격주 단일 일정, `0..30`일 선행 생성과 중복 없는 스케줄러 재실행, 활성 반복 업무가 없을 때 발생 커서만 전진하고 빈 자동 회차를 만들지 않는 처리
- 시즌 이름·기간 수정, 명시적 종료·재개와 선택한 역할·활성 반복 업무 정의를 새 UUID 스냅샷으로 복사하는 멱등한 다음 시즌 시작 API
- 멱등한 공유 키 회전과 별도 파일럿 복구 키를 이용한 분실 복구
- MySQL 영속화와 Flyway 마이그레이션
- 역할 자료·시즌 트랜잭션과 함께 저장하는 WATCH 모니터 아웃박스, 커밋 이후 전용 스케줄러의 임대·재시도 전달, 시작 시 운영 실패 복구와 동시 변경을 되돌리지 않는 현재 상태에 맞춘 재동기화
- 업무·인수인계 점검 결과를 원본 트랜잭션과 함께 저장하는 BRIEF 이벤트 v2 불변 outbox, 커밋 이후 전용 스케줄러의 임대·신호별 리비전 순서·재시도 전달 기반
- 별도 Bearer로 보호한 `POST /api/v1/internal/resource-health-events`, 이벤트 ID별 원자적 불변 인박스와 신규·동일 재전송의 `202` 접수증, 같은 ID의 다른 내용 `409` 처리
- 애플리케이션 경계의 공유 키 검증, 원문 키 비저장과 구성원·역할·반복 업무 정의·시즌 회차·회차 실행·역할 자료·결정·인수인계 항목·역할 인수인계의 겹친 수정 충돌 처리
...
현재 파일럿은 계정 세션과 기존 워크스페이스 접근 권한을 분리한다. 로그인 신원·기존 구성원 연결·ROUND 참여권은 계정 세션으로, 기존 제품 경로는 애플리케이션의 공유 키 검증으로, WATCH 내부 이벤트 경로는 별도 Bearer로 보호한다. 팀별 계정 전환과 초대·ADMIN·MEMBER·VIEWER 권한은 PRD-0009를 따르며, 시즌·역할별 세부 권한은 후속 범위다. 첫 파일럿 배포는 Docker Compose와 Caddy를 사용하는 단일 호스트 동일 출처 HTTPS 구성을 제공하지만, 장기 운영 공급자와 확장 토폴로지는 아직 결정하지 않았다.

### 연관 마이크로서비스 경계

BATON 본체는 조직·시즌·역할·업무 기록과 최종 접근 권한을 관리한다. 다음 서비스는 각각 독립 저장소·런타임·배포 단위를 유지한다. 구현·검증 단계는 서비스마다 다르므로 아래 현재 상태를 기준으로 판단한다.
...
- `BATON WATCH`: 역할 자료 URL 스냅샷의 비동기 상태 점검, SSRF 방어, 임대·시도·결과·현재 연결 상태와 상태 변경 이벤트 전달을 소유한다. BATON은 점검 대상 자료와 시즌 상태의 변경을 불변 트랜잭셔널 아웃박스에 기록하고 기능을 활성화한 뒤 커밋 이후 WATCH 모니터로 전달·재조정한다. WATCH가 최소 한 번 전달 방식으로 보낸 이벤트는 별도 인증의 트랜잭셔널 인박스에 원자적으로 수신하지만, 역할 상세에서 별도 현재 상태 조회와 재점검 UI를 제공한다. 이벤트 기반 상태 프로젝션, 실제 공개 스테이징의 WATCH→BATON 전달·동일 재전송과 운영 활성화는 아직 완료하지 않았다.
- `ROUND`: WebRTC 방·피어·시그널링과 TURN 자격 증명 발급을 소유한다. BATON은 AccountMembership과 서버에서 관리하는 방 매핑을 바탕으로 짧은 수명의 참여권을 발급한다. 선택 실행 교차 서비스 테스트는 실제 BATON 서명자와 ROUND `bootJar` 사이의 발급자·단일 수신자·JWK 회전과 TURN·WebSocket 방 경계를 검증한다. 기본 전 구간 테스트는 테스트 전용 자체 이메일 계정의 실제 브라우저 로컬 세션에서 기존 구성원을 연결하고 방 매핑·참여권 쿠키·공개 JWK와 서명까지 검증한다. 별도 선택 실행 경계 테스트는 로컬 사설 CA의 테스트 전용 Caddy와 기존 ROUND 웹·시그널링 이미지를 연결해 같은 브라우저의 `Secure` 쿠키로 TURN 자격 증명을 받고 WSS 방에 입장하는 공개 경로를 검증한다. 프로덕션 Caddy·Compose에는 선택 실행 런타임과 자격 증명 최소 전달 경계를 반영했으며, 실제 릴리스 다이제스트·외부 coturn을 사용한 공개 스테이징 검증은 남아 있다.
- `BATON GO`: 공개 링크 코드의 시간·폐기와 BATON·ROUND 신뢰 대상 라우팅을 소유한다. 워크스페이스와 방의 최종 접근 권한은 각 소유 서비스가 계속 판단한다.
- `BATON BRIEF`: BATON의 업무·인수인계 점검 이벤트를 수신해 중복 처리를 막고, 점검 항목과 생성 당시 내용을 보존한 주간 요약을 관리한다. BRIEF 이벤트 v2와 [현재 계약 팩](contracts/brief/README.md)을 BATON에 고정해 Java/Jackson 직렬화를 검증했고, 신호별 현재 상태·연속 리비전과 불변 outbox를 기록하는 트랜잭션 재조정과 설정형 시간 스케줄러를 구현했다. 신호에 영향을 주는 원본 변경과 자동 회차 생성은 같은 트랜잭션에서 재조정해 원본과 outbox를 함께 커밋한다. V25는 커밋 뒤 lease·신호별 순서·재시도 전달 생명주기를 추가하고 실제 이벤트 record를 BRIEF `POST /api/v1/events`로 직렬화한다. 선택 실행 교차 서비스 테스트는 실제 두 실행 JAR과 MySQL·PostgreSQL에서 원본 API 변경, 초기 데이터 동기화, 장애 재시도, 동일 이벤트 재전달, 심각도 변경·해결 상태 반영과 전용 Bearer 인증을 검증한다. BRIEF가 새 token과 직전 token을 함께 허용한 교체 구간의 전달도 확인했으며 HTTPS·스테이징 활성화는 아직 완료하지 않았다.
...
역할 상세의 활성 자료에는 WATCH 연결 상태, 최근 상태 확인·점검 시도 시각과 다시 점검 버튼을 표시한다.
자료 조회와 별도 요청을 사용하며 WATCH 장애·누락·5분 이상 오래된 연결 판정은 자료 링크를 유지한 채
확인 필요로 표시한다. 로그인 후 접근 권한은 검사하지 않는다. 상태 조회·재점검에는 기존
`BATON_WATCH_ENABLED`, WATCH HTTPS 주소·API 토큰·소스 이름공간 설정을 사용한다.
새 마이그레이션은 없으며 WATCH 재점검 API가 포함된 `3a04e7b` 이상을 먼저 배포한다.
여러 자료 조회를 나누어 보내고 같은 자료의 진행 중인 원격 조회를 공유한다. 재점검 버튼은
다시 요청할 수 있는 시간까지 남은 초를 표시하며, 새 연결 판정을 확인하면 접수 안내를 바꾼다.
내부 오류로 시도 시각만 바뀌어도 이전 판정의 5분 유효 기간은 늘어나지 않는다.
최근 결과에는 점검 실패 원인과 연속 연결 실패 횟수도 표시한다. 오래된 결과와 조회 장애에는
이 정보를 표시하지 않는다. 바통 전달·수정 충돌로 편집이 잠겨도 상태는 계속 조회하며,
이때 다시 점검 버튼은 숨긴다. 자동 점검 대상 여부는 서버의 응답으로 표시한다.
연동 중지·감시 일시 중지·시즌 종료·보관·URL 조건 미충족·감시 비활성 사유를 구분하고,
최초 등록이나 주소 반영이 끝나지 않은 자료는 동기화 대기로 표시한다. 재점검 처리 중 표시와
남은 대기시간은 자료별로 유지하므로 다른 역할을 보고 돌아와도 중복 요청을 막는다.
접수·새 결과 확인 안내도 역할 이동 뒤 유지하며 확인 이력은 오프라인이나 조회 실패로
되돌리지 않는다. 새 재점검과 URL·공유 키 변경 시 이전 안내를 초기화한다.
오프라인이거나 조회 캐시가 30초를 넘으면 이전 정상·실패 상태를 현재 상태로 표시하지 않는다.
탭 복귀와 네트워크 재연결 때 상태를 다시 조회하며, `상태 새로고침` 버튼으로도 확인할 수 있다.
이 버튼은 최근 결과를 읽으며 URL 점검을 새로 접수하지 않는다. 스크린리더에는 수동
조회의 진행·결과와 표시 상태 변경을 알리며 같은 상태의 자동 조회는 반복해서 알리지 않는다.
공개 운영 검증과 콜백 활성화는 별도로 진행한다.
[상태 조회 결정](docs/ADR/0021_watch-current-state-query/adr.md)과
[WATCH 연동 계약](docs/PRD/0004_watch-integration-contract/spec.md)을 참고한다.
...
1. `http://127.0.0.1:3000`에서 팀 이름, 시즌 기간과 구성원을 입력한다. 이름이 같은 구성원은 별칭으로 구분한다. 서버에 생성 코드가 설정되어 있으면 파일럿 생성 코드도 입력한다.
2. 역할 화면에서 구성원을 추가하고 이름·활동 상태, 역할과 반복 업무·마감을 등록한다. 역할 상세에는 함께 사용할 문서 링크를 연결한다. 활동이 끝난 구성원의 기존 역할·결정은 유지하며, 새 담당자·작성자로는 선택할 수 없다.
3. 오늘 화면의 운영 점검에서 담당자·후임 누락, 인수인계 준비 부족과 반복 지연의 원인·조치 방법을 확인한다.
4. 일정 화면에서 시즌 시간대와 주간·격주 일정을 설정한다. 회차는 미리 자동 생성하거나 수동으로 만들 수 있다. 수동·자동 회차의 이름·날짜를 수정하고, 활성 회차의 반복 업무를 완료 처리한다.
5. 검색 화면에서 결정 이유, 관련 역할, 인수인계 항목과 자료를 찾는다. 역할·상태·시즌 시간대 기준 기간으로 결과를 좁힐 수 있다. 회차·결정·인수인계 항목은 보관 후 복원해도 기존 실행 기록과 상태가 유지된다.
6. 역할을 교대할 때는 인수인계 화면에서 다음 담당자와 담당 기간을 정한다. 누락 경고를 확인하고 전달한 뒤 다음 담당자 명의로 수락한다.
7. 시즌이 끝나면 진행 중인 역할 인수인계를 수락하거나 취소한다. 상단 시즌 전환에서 다음 시즌을 시작하고, 이어 갈 역할과 활성 반복 업무를 선택한다. 담당자·담당 기간, 회차 일정과 실행 상태는 새 시즌에서 다시 정한다. 과거 시즌은 읽기 전용이며 모든 시즌 검색에도 포함된다.
8. 사이드바나 모바일 상단의 공유 기능으로 링크를 복사해 구성원에게 전달한다. 접근 키가 포함된 공유 링크에는 작업 공간의 읽기·쓰기 권한이 있으므로 공개 채널에 게시하지 않는다.

더 이상 사용하지 않는 반복 업무는 보관하면 새 수동·자동 회차와 다음 시즌에서 제외된다.
보관 전 회차의 업무는 계속 완료 처리할 수 있으며, 복원한 업무는 다음 회차부터 포함된다.
모든 반복 업무를 보관하면 자동 일정은 빈 회차를 만들지 않고 다음 생성 시점으로 넘어간다.
화면에는 `회차에 추가할 업무 없음`으로 표시한다.

접근 키 원문은 작업 공간 생성·키 교체·복구의 최초 응답과 같은 멱등 요청의 응답 유실 복구 때만 반환한다.
서버에는 SHA-256 해시만 저장한다. 키가 외부에 알려졌다면 작업 공간의 `공유 링크 관리`에서 재발급하고
새 공유 링크를 전달한다. 서버는 사용한 키 변경 요청의 멱등 해시를 기억하므로,
과거 요청을 다시 보내도 최신 키 변경으로 폐기된 링크가 되살아나지 않는다.

구성원·역할·반복 업무·회차·결정·인수인계 항목·역할 자료 생성이나 역할 인수인계 준비 후
응답을 받지 못하면 브라우저에 보관한 같은 멱등 키로 재시도한다. 이미 생성된 항목을 반환하므로
중복 생성되지 않는다. 회차를 재조회하면 최초 회차와 실행 기록의 식별자는 유지하되,
그 후 수정·보관·완료 처리한 내용은 현재 상태로 반환한다.
자료 URL은 사용자 정보가 없는 `http` 또는 `https` 전체 주소만 허용한다.
BATON 서버는 링크 대상의 내용이나 신뢰성을 확인하지 않는다.

브라우저 저장소가 차단되어 복구용 멱등 키를 보관할 수 없으면 작업 공간·콘텐츠 생성과
키 교체를 시작하지 않는다. 여러 탭의 생성 요청을 한 번에 하나씩 처리하는 잠금을 지원하지 않아도
작업 공간·콘텐츠 생성을 시작하지 않는다. 일반 브라우징 모드에서 사이트 저장소를 허용하고
최신 브라우저를 사용한다.

모든 구성원이 키를 잃었다면 운영자가 추측하기 어려운 무작위 멱등 키를 생성하고 아래 복구 API를 호출한다.
기존 키는 폐기되고 새 키가 발급된다. 응답을 받지 못했다면 멱등 키를 바꾸지 않고 같은 요청을 재시도한다.
...
운영 환경 설정은 주석과 검증기가 허용한 단순한 `KEY=VALUE`만 사용한다.
따옴표, 공백, `$` 보간과 포트 게시 재정의를 넣지 않는다. 공통 검증기는 다음을 확인한다.

- 현재 사용자 소유의 일반 파일이며 그룹·기타 사용자의 접근 권한과 Git 추적이 없는지
- 호스트가 공개 DNS 이름 형식이며 DB 식별자가 유효한지
- 비밀값이 32~200자의 URL 안전 문자이고 서로 다른지

`preflight-production.sh`는 Docker 데몬·Compose v2와 최종 Compose 설정도 확인한다.
ROUND가 활성화되면 지정한 다이제스트의 이미지를 받아 두 이미지의 릴리스 리비전·태그 객체가
일치하는지, 웹 이미지가 BATON 모드인지 검사한다.

`production-compose.sh`는 `up`, `create`, `pull` 실행 시 배포 잠금을 잡고 환경 설정의 복사본을 만든다.
이 파일은 잠금 디렉터리에 `0600` 권한으로 보관하며 환경 설정, 이미지 다운로드와 호환성을 다시 검증한다.
검증과 Compose 실행은 이 복사본만 사용하고 종료 시 삭제한다. 실행 중 원본 설정이 바뀌어도
해당 실행에는 반영되지 않는다.

운영자가 지정한 다이제스트로 이미지를 고정하지만, 게시자 서명이나 빌드 출처는 검증하지 않는다.
실제 DNS 연결, 외부 80/443 접근, 공인 인증서 발급과 호스트 디스크 여유도 별도로 확인해야 한다.

`production-compose.sh`는 모든 명령 직전에 환경 설정을 다시 검증하고,
배포·Compose 설정과 충돌할 수 있는 셸 환경 변수를 제거한다.
프로젝트는 `baton-production`, Compose 파일은 저장소의 프로덕션 설정을 사용한다.
Docker 연결 주소는 Linux 로컬 `unix:///var/run/docker.sock`으로 고정한다.

서비스 상태를 바꾸거나 프로덕션 이미지를 가져오는 명령은
`/srv/baton/state/production-lifecycle.lock` 파일의 `flock`을 잡는다.
환경 설정·체크아웃·호출 UID가 달라도 같은 파일의 아이노드를 잠그므로 `restore.sh`와 동시에 실행되지 않는다.
잠금 상위 디렉터리와 파일은 운영 사용자만 접근하도록 각각 `0700`, `0600`이어야 한다.

`up`과 `create`는 일부 서비스만 지정해도 활성화된 전체 서비스를 배포 설정에 맞추고
`--remove-orphans`로 제외된 컨테이너를 정리한다. 명령은 운영에 필요한 허용 목록으로 제한한다.

- 변경한 설정·기능 활성화 여부·Compose 오버레이를 반영하지 않는 `start`, `restart`, `pause`,
  `unpause`, `watch`, `up --watch`와 대화형 메뉴는 거부한다. `COMPOSE_MENU=false`로 고정한다.
- 독립 실행 `build`는 허용하지 않는다. 배포 빌드는 전체 서비스를 반영하는 `up --build`로 실행한다.
- `run`, `attach`, 모델 변환, 이미지 게시, 확장, 데이터 볼륨 삭제를 거부한다.
  호출자가 파일·프로필·프로젝트를 바꾸거나 고아 컨테이너 정리·재생성을 우회하는 옵션도 거부한다.
- `config`는 비밀값을 출력하지 않는 `--quiet`만 허용한다.

사전 점검 후 환경 설정의 내용·권한·Git 추적 상태가 잘못 바뀌면 다음 Compose 명령은 실패한다.
다른 환경 설정 파일은 `./ops/preflight-production.sh /absolute/path/to/env`로 먼저 검사하고,
모든 Compose 명령에 `BATON_PRODUCTION_ENV_FILE=/absolute/path/to/env`를 지정한다.

`BATON_HOST`, DB 사용자·비밀번호, `BATON_WORKSPACE_CREATION_KEY`와
`BATON_WORKSPACE_RECOVERY_KEY`가 빠지면 프로덕션 Compose는 설정 단계에서 실패한다.
Compose 없이 직접 실행할 때도 설정한 두 운영 비밀은 32~200자의 URL 안전 ASCII여야 한다.
`production`에서는 두 값이 모두 있고 서로 달라야 애플리케이션이 시작된다.
프로덕션 프로젝트 이름과 DB 볼륨은 `baton-production`으로 고정해 로컬 Compose 데이터와 분리한다.
MySQL은 호스트 포트를 열지 않고 애플리케이션과 내부 TLS로 통신한다.
...
- `briefCrossServiceTest`: 실제 BATON·BRIEF 실행 JAR과 MySQL 8.4·PostgreSQL 18.6을 연결해 이벤트 초기 데이터 동기화·장애 재시도·동일 재전달·해소 투영을 검증하고, 별도 서비스 Caddy·PKCS12 truststore·`Internal=true` 네트워크에서 사용자 세션 기반 생성본 생성·조회·응답 유실 재시도와 서비스 token 교체를 검증하는 선택 실행 테스트
...
교차 서비스 테스트는 기본 `test`·`build`에 외부 저장소를 암묵적으로 결합하지 않는다. BRIEF 테스트는 미리 빌드한 BRIEF 실행 JAR의 절대 경로를 `briefBootJar` 속성 또는 `BRIEF_BOOT_JAR` 환경 변수로 받아 BATON 실행 JAR은 현재 저장소에서 빌드한다. 이벤트 응답 유실은 BRIEF 수신 뒤 BATON 전달 행을, 생성본 생성 응답 유실은 BRIEF 저장 뒤 BATON 실행 성공 상태를 각각 재시도 상태로 되돌려 재현한다. 실제 TCP 응답 절단은 아니며 로컬 CA 결과를 공인 HTTPS 완료로 해석하지 않는다. `ROUND_REPOSITORY_ROOT`를 생략하면 BATON과 같은 상위 디렉터리의 `webRTC`를 사용하며, 이미 빌드한 JAR를 재사용하려면 `ROUND_SIGNALING_JAR` 절대 경로만 지정한다. 두 값은 동시에 사용할 수 없고 실행 로그에는 실제 검증한 JAR와 저장소를 사용한 경우 Git 리비전·변경 상태가 남는다. ROUND 교차 서비스 경계는 실제 BATON 서명자와 ROUND의 Nimbus JWK 디코더·키 회전·캐시 누락·갱신 제한·쿠키·방 결속을 검증하며, 고정 시각 Nimbus 소스 테스트가 JVM 캐시의 60초 만료와 30초 구간당 소스 접근 상한을 별도로 고정한다. 이 ROUND 경계에는 BATON 세션·AccountMembership·공개 Caddy TLS 경로와 실제 SMTP 가입이 포함되지 않는다.
...
- 개인 캘린더 구독: 오늘 화면의 **내 캘린더에 추가**에서 활성 구성원이 자기 시즌의 구독 주소를
  ...
  진행 중에는 남은 요청을 중단할 수 있으며 이미 보낸 요청은 서버에서 완료될 수 있다.
...
- BATON 경유 주간 요약 조회·생성 계약: [PRD-0008](docs/PRD/0008_brief-edition-query-and-generation/spec.md)
- BATON 경유 점검 항목 요약·필터: [PRD-0009](docs/PRD/0009_brief-current-attention/spec.md)
