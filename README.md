...
역할 자료 상세에서 사람이 확인한 최근 20건을 조회하고, 로그인 계정과 연결된 활성 구성원이 사용 가능·수정 필요와 500자 이내 메모를 남길 수 있다. 서버는 확인 당시 구성원 이름·자료 주소·자료 버전·시각을 보존한다. 자료를 수정하거나 보관·복원해 버전이 바뀌면 이전 확인으로 표시하며 새 확인이 필요하다. 종료 시즌·보관 자료에는 확인을 추가할 수 없다. WATCH 자동 점검 상태를 뜻하지 않으며 공유 키 팀의 계정·구성원 연결은 키 소유자가 직접 선택하며 계정 전환 팀은 관리자의 승인이 필요하다.
