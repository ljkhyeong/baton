...
작업 공간의 `공유`, 구성원 관리의 `초대 링크 공유`, 주간 요약의 `요약 공유`를 [Web Share API](https://www.w3.org/TR/web-share/)에 연결했다. 사용자가 기기의 공유 창에서 메신저·메일 등 설치된 앱을 선택한다. 브라우저 표준 기능이므로 BATON에 공급자 SDK·계정·키나 유료 API를 추가하지 않는다.

공개 환경에서는 HTTPS가 필요하며 브라우저·운영체제에 따라 지원 여부와 표시되는 앱이 다르다. 미지원 기기는 링크를 복사하고, 복사도 차단되면 직접 복사할 주소를 제공한다. 공유 창 호출이 실패하면 작업 공간은 복사로 전환하고, 초대·요약은 기존 복사 버튼이나 주소를 사용하도록 안내한다. 취소하거나 공유 대상 앱이 없으면 복사하지 않는다. 앱 선택만으로 수신자에게 전달됐다고 표시하지 않는다.

초대 링크는 생성한 초대가 유효한 동안만 공유할 수 있다. 주간 요약에는 선택한 요약의 주소만 전달하며 팀 접근 키·본문·비교 결과를 넣지 않는다. 두 화면 모두 공유 창을 여는 동안 공유·복사 버튼을 막는다. 기존 복사 버튼은 기기 공유 지원 여부와 관계없이 사용할 수 있다.
