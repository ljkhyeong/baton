...
인박스는 저장과 중복 제거까지만 수행한다. [ADR-0021](../0021_watch-current-state-query/adr.md)의 상태 UI는 별도 현재 상태 조회를 사용하며 인박스를 소비해 상태를 덮어쓰지 않는다. 처리 완료 상태와 보존 기한에 따른 삭제는 추가하지 않는다.
...
WATCH가 전달 순서를 보장하지 않아 오래된 이벤트가 최신 상태를 덮을 수 있다. 현재 상태 프로젝션에는 별도의 단조 상태 순번, WATCH 상태 대조 또는 그에 준하는 순서 계약이 필요하므로 채택하지 않는다.
