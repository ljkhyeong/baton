...
- 프런트엔드는 `npm run build`로 엄격한 TypeScript 검사와 프로덕션 번들을 확인하고 Chromium·WebKit을 설치한 뒤 Playwright E2E를 실행한다. Chromium은 전체 흐름과 390px 모바일 흐름, WebKit은 핵심 스모크·반응형 흐름을 소유한다. CI에서는 `test.only`를 거부하고 재시도에서만 성공한 불안정 테스트도 실패로 판정하며, 단일 워커로 실행 특성을 고정한다. 실패한 실행의 보고서·추적·화면 캡처는 7일 동안 산출물로 남긴다.
