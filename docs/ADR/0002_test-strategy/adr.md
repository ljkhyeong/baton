...
루틴 정의 보관은 V14 데이터를 V15로 올려 기존 정의·실행 계보와 version을 보존하고 `archived_at`을 활성 상태인 `null`로 초기화하는 전용 migration 테스트를 둔다. 보관 전 실행 보존, 이후 수동·자동 회차 제외, 복원과 다음 시즌 선택 경계는 유스케이스 테스트로 함께 고정한다.
