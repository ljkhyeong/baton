...
로그인한 자체 이메일 계정의 비밀번호 변경도 같은 `sessionVersion` 경계를 사용한다. 현재
비밀번호는 Spring Security `PasswordEncoder`로 확인하고, 새 비밀번호 저장과 세션 버전 증가를
한 트랜잭션에서 끝낸다. 성공하면 아직 사용하지 않은 비밀번호 재설정 챌린지와 대기 중인 메일을
폐기한다. 명시적인 모든 기기 로그아웃은 비밀번호를 바꾸지 않고 세션 버전만 증가시킨다. 두 경로는
Spring Security 로그아웃 핸들러로 현재 보안 컨텍스트·세션·`JSESSIONID`도 즉시 제거한다.
...
- 선택형 Turnstile은 가입·비밀번호 재설정 메일 요청에서 Siteverify의 허용 호스트와 요청별
  `action`을 확인한다. 브라우저 token만 신뢰하지 않으며 기존 요청률 제한을 함께 유지한다.
