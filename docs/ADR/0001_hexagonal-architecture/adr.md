...
- `domain`은 application, adapter와 bootstrap을 참조하지 않는다.
- `application`은 adapter와 bootstrap을 참조하지 않는다.
- inbound web adapter는 outbound adapter와 `port.out`을 직접 참조하지 않고 `port.in`을 호출한다.
- outbound adapter끼리 직접 결합하지 않는다.
- `bootstrap`은 composition root로서 모든 모듈을 조립할 수 있다.
...
현재 `domain`은 `jakarta.persistence-api`를 `api` 의존성으로 허용한다. 초기 서비스의 모델과 JPA 매핑을 가깝게 유지해 구현 비용을 줄이기 위한 선택이다. 이 결정 때문에 domain이 완전한 프레임워크 독립 모델은 아니지만, repository와 데이터 접근 구현은 계속 persistence adapter가 소유한다.
...
- 기능의 진입점과 출력 의존성이 port로 드러난다.
- 단일 배포 단위를 유지하면서 모듈 경계를 자동 검사할 수 있다.
- 테스트에서 application 흐름을 실제 adapter와 조립하기 쉽다.

### 비용

- 작은 기능도 port와 adapter를 나누는 파일 비용이 생긴다.
- 테스트용 조립 의존성과 프로덕션 의존성을 구분해서 읽어야 한다.
- JPA annotation을 domain에 허용한 절충을 의식적으로 관리해야 한다.
