...
한 tick은 활성 일정 후보 전체를 식별자 순으로 읽고, 각 시즌에서 최대 8개의 밀린 발생을 처리한다. 시즌 수에 고정된 앞쪽 cap을 두지 않아 뒤쪽 일정이 영구적으로 굶지 않게 한다. 시즌별 실패는 식별자와 원인을 로그에 남기고 다음 시즌 처리를 계속하며, 시즌별 처리 한도에 남은 발생은 다음 tick에서 이어 간다. 모든 후보를 처리한 뒤 실패가 하나라도 있으면 use case가 실패 시즌 목록을 반환하고 scheduler 진입점은 예외로 종료해 Spring의 scheduled-task 관측이 성공으로 왜곡되지 않게 한다. 다음 tick 실행과 시즌별 격리는 그대로 유지한다.
...
시간대·주간·격주·선행 생성 경계, 일정 일시 중지와 커서 보존, 실제 마감 스냅샷, 정확한 마감 instant의 지연 전환, DST gap·overlap, 기존 발생의 재실행, 시즌별 실패 격리와 실패한 tick의 관측 상태, tick 처리 한도를 확인한다. MySQL에서는 V11 데이터가 V12의 nullable 일정·마감과 자동 회차 유일 제약으로 안전하게 이관되는지 확인한다.
