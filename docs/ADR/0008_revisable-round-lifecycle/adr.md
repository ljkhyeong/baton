...
- 회차 보관·복원은 같은 `SeasonRound` 부모 행의 비관적 쓰기 잠금을, 실행 완료 변경은 공유 읽기 잠금을 먼저 얻고 트랜잭션이 끝날 때까지 유지한다. 완료 요청끼리는 부모 공유 잠금을 함께 통과하므로 같은 실행을 겹쳐 바꾸면 `RoutineExecution.@Version`이 한쪽을 `409 WORKSPACE_CONTENT_CONFLICT`로 거절한다.
