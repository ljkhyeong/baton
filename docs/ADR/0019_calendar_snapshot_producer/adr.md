...
- 시즌 이름은 `1.1.0-rc.1` 로컬 후보를 명시한 선택 실행 테스트로만 검증한다. 안정 계약 핀은
  유지하며, 기존 시즌 보정·복원 재전달과 정식 계약 채택은 남아 있다. 이름 설정을 끈 기간의
  변경은 자동 보정되지 않으므로 기존 시즌·복원 절차를 구현하기 전에는 운영에서 활성화하지 않는다.
