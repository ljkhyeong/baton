...
다음 구현 단계에서는 원본 변경과 같은 트랜잭션에서 불변 CAL 아웃박스를 추가하고 커밋 후 HTTP로
전달한다. 아웃박스의 `INT AUTO_INCREMENT` 키를 CAL 개정 번호로 사용해 JPA `@Version`과 외부
순서를 분리한다. 각 행은 전체 스냅샷을 보존하고 같은 행 재시도에서 이벤트 ID와 페이로드를 바꾸지
않는다. 이전 미종결 행이 있는 같은 원본의 후속 행은 먼저 임대하지 않는다.
...
- 다음 단계의 트랜잭셔널 아웃박스가 사용할 안정적인 애플리케이션 모델을 확보한다.

### 비용과 한계

- 계약 스키마 사본과 핀 정보를 BATON 저장소에서 갱신해야 한다.
- 현재는 직렬화 경계만 구현했으며 아웃박스, HTTP 전달, 인증, 재시도와 기존 데이터 조정은 없다.
