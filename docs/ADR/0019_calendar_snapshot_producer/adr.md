...
1마이크로초 전진시킨다. 캡처는 기본 비활성이고 HTTP 전달 작업자를 구현하기 전에는 운영에서 켜지
않는다. 후속 전달 작업자는 이전 미종결 행이 있는 같은 원본의 다음 행을 먼저 임대하지 않는다.
...
- 현재는 HTTP 전달, 인증, 재시도와 기존 데이터 조정이 없다. 캡처를 켜면 전달되지 않은 행만
  쌓이므로 운영에서는 계속 비활성으로 둔다.
