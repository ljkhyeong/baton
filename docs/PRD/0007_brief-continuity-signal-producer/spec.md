# PRD-0007: BATON–BRIEF 업무 점검 이벤트 발행 계약

- 상태: 채택
- 결정일: 2026-08-22
- 수정일: 2026-08-27
- 구현 상태: BRIEF 이벤트 v2·RC 계약 팩·직렬화, 이벤트 이력·아웃박스, 주기적 재계산·원본 변경 연동, 커밋 후 HTTP 전송과 운영 설정 구현. 로컬에서 원본 API 변경부터 BRIEF 수신까지 검증 완료
- 범위: BATON의 업무·인수인계 점검 이벤트를 DB에 저장하고 BRIEF로 전달하기 위한 판정·식별·리비전·상태 갱신 기준

## 1. 목적

BATON은 팀·시즌·역할·반복 업무·인수인계 기록을 관리하고 업무 문제를 판정한다. BRIEF는 이 기록을 직접 읽거나 다시 판정하지 않는다. BATON이 커밋 후 보낸 이벤트를 중복 없이 처리해 점검 항목과 변경되지 않는 주간 요약을 만든다.

워크스페이스 응답의 `continuitySignals`는 조회 시점의 `Clock`과 시즌 시간대로 계산한다.
조회 결과를 그대로 보내면 같은 항목을 식별하거나 해결 상태·리비전·날짜 변경을 추적할 수 없다.
이 문서는 점검 기준과 이벤트 저장·전달 범위를 정한다.
...
| 생명주기 | 조회 시 계산한 현재 목록 | 안정적인 `ACTIVE`, `RESOLVED` 상태 이벤트 | 이벤트 이력 저장과 재계산을 추가하기 전에는 전송하지 않는다. |
| 리비전 | JPA `@Version`은 내부 충돌 제어 | 신호 식별 기준별 양의 단조 증가 값 | 외부 신호 리비전을 별도로 소유한다. |
...
| `ROLE_PREPARATION_INCOMPLETE` | 시즌·역할 | 역할 위험이 있고 책임·활성 인수인계 항목·자료 준비가 부족하며 역할 인수인계 신호가 없음 | 필요한 준비 충족, 위험 제거, 역할 인수인계 신호로 전환 또는 시즌 종료 |
| `ROUTINE_REPEATEDLY_OVERDUE` | 시즌·역할·반복 업무 | 보관하지 않은 반복 업무가 서로 다른 두 회차 이상에서 지연됨 | 지연 회차 감소, 반복 업무 보관 또는 시즌 종료 |
| `HANDOFF_INCOMPLETE` | 시즌·역할 | 열린 역할 인수인계의 참여자·준비·전달·담당 기간 공백 또는 임박한 인수인계 미시작 문제 | 문제 해소, 역할 인수인계 수락·취소 또는 시즌 종료 |

같은 신호가 유지되는 동안 심각도나 BRIEF가 소비하는 근거가 바뀌면 같은 식별 기준의 다음
리비전을 만든다. 제목, 이유와 권장 행동은 BATON 화면 설명이며 외부 신호 식별자로
사용하지 않는다.

## 4. 이벤트 v2 경계

BRIEF는 다음 의미의 이벤트 v2를 구현했고, BATON은 `2.0.0-rc.4` 계약 팩을 저장소에
고정했다. BATON 송신기는 기본 비활성 상태이며 전용 아웃박스·커밋 뒤 전달 생명주기와
BRIEF 출처를 명시한 환경에서만 켠다.
...
전역 아웃박스 자동 증가 번호는 다른 신호 때문에 건너뛰므로 `aggregateRevision`으로 사용하지
...
영속 스트림·아웃박스·전달과 종단 간 수신 성공을 증명하지 않는다.

## 5. 점검 이벤트 이력 저장

생산자 구현은 신호 식별 기준과 영속 `signalId`, 마지막 상태·심각도, 신호별 최신 리비전을
...
현재 신호 계산, 기존 상태 비교, 현재 상태 갱신과 불변 아웃박스 삽입을 같은 트랜잭션에서
수행한다. 원본 변경 뒤에만 신호 행을 잠그면 MySQL `REPEATABLE READ`의 이전 원본
스냅샷과 최신 신호 상태가 섞일 수 있으므로, 첫 원본 조회 전에 시즌 잠금을 얻어야 한다.

신호에 영향을 주는 시즌·구성원·역할·역할 인수인계·반복 업무 보관·회차·
실행·인수인계 항목·역할 자료 변경은 이 경계를 같은 원본 변경 트랜잭션에서 호출한다. 자동
회차를 실제로 생성했을 때도 시즌 저장 뒤 같은 트랜잭션에서 재조정한다. 이미 생성된 발생일이거나
활성 반복 업무가 없어 커서만 전진한 경우에는 재조정하지 않는다. 신호 의미를 바꾸지 않는 접근 키,
구성원 이름, 결정과 반복 업무 설명 변경에는 불필요한 재조정을 붙이지 않는다.
일반 시즌 수정은 시작일 또는 종료일이 바뀔 때만 재조정하고, 기간이 같은 이름 수정은 제외한다.
운영자 시즌 이름 정정 API도 신호 판정 입력을 바꾸지 않으므로 재조정에서 제외한다.
자동 회차 설정은 정규화된 시즌 시간대가 바뀔 때만 재조정한다. 시간대가 같으면 모임 날짜·시각,
반복 주기·선행 생성 기간·활성 여부를 변경해도 재조정을 생략하며, 실제 회차 생성 시의 재조정은 유지한다.

시즌 종료·재개, 구성원 활동 상태, 반복 업무·회차·인수인계 항목·역할 자료의 보관·복원과 실행·인수인계 항목의
완료 여부는 저장된 상태가 실제로 바뀔 때만 재조정한다. 같은 상태를 다시 요청해도 기존 권한·소유권,
시즌 잠금과 편집 제한 검사는 그대로 수행한다.
역할 자료의 제목·URL·설명과 인수인계 항목의 문구·분류 수정은 같은 역할 안에서는 신호 판정 입력을
바꾸지 않으므로 재조정하지 않는다. 다른 역할로 이동하면 재조정하며, 자료 URL 변경의 WATCH 기록과
회차 보관·복원의 CAL 기록은 기존 경로를 유지한다. 변경 전 값은 원본을 이미 조회한 코드에서 비교하고
BRIEF 기록까지 같은 원본 변경 트랜잭션으로 처리한다.

수동·자동 회차 수정은 모임 날짜가 바뀔 때만 실행 마감을 다시 계산·저장하고 BRIEF를 재조정한다.
날짜가 같은 이름 수정에서는 저장된 실행 마감을 유지한다. 회차 이름을 반영하는 CAL 기록과
기존 권한·잠금·편집 제한 검사는 날짜 변경 여부와 관계없이 유지한다.

역할·수동 회차·인수인계 준비·인수인계 항목·역할 자료 생성은 실제 저장 직후에만 BRIEF를 재조정한다.
같은 멱등 키와 요청으로 기존 결과를 반환할 때는 재조정하지 않는다. 다음 시즌 생성도 실제 생성
경로에서 이전 시즌의 신호 해소와 새 시즌의 신호 계산을 모두 처리하고 재전송에서는 둘 다 생략한다.
기존 멱등 키·요청 일치 검사, 권한·소유권과 트랜잭션 잠금, CAL·WATCH 기록은 유지한다.

역할 인수인계의 전달·수락·취소도 실제 상태 저장 뒤에만 BRIEF를 재조정한다. 같은 확인자 명의로
이미 완료한 요청을 반복하면 기존 재요청 판정에 따라 현재 결과만 반환한다. 수락·취소 후에
과거 전달 요청을 다시 보내는 경우에도 BRIEF 재계산을 생략한다.
...
4. 같은 `eventId`와 이벤트 본문을 가진 전용 BRIEF 아웃박스 행을 함께 기록한다.

동일 상태를 반복 계산하면 새 리비전과 아웃박스 행을 만들지 않는다. 해소된 같은 자연
정체성이 다시 발생하면 기존 `signalId`에서 더 큰 리비전의 `ACTIVE`를 기록한다. 생산자와
재생 계약을 유지하는 동안 스트림 식별 기준과 리비전 근거를 임의로 삭제하거나 재기준화하지
않는다.
...
- 원본 변경 트랜잭션 뒤 점검 항목 재계산
- 날짜·마감 경계를 발견하는 주기적 재계산과 첫 활성화 시 기존 상태 일괄 반영

재계산은 주입 `Clock`과 시즌 IANA `timeZone`을 사용한다. 같은 고정 시각과 같은 원본
스냅샷에서는 결과와 아웃박스가 멱등해야 한다. 구체적인 폴링 주기, 페이지 크기와 실행 시간
...
남은 종료 시즌의 합집합이며, 종료 상태 재계산으로 마지막 `RESOLVED`를 기록한 뒤에는 후보에서
빠진다. 종료 시즌은 신호 계산용 구성원·역할·반복 업무·회차·실행·인수인계·자료를 조회하지 않고,
저장된 활성 신호의 해소만 처리한다. 후보 조회는 읽기 전용이고 각 시즌은 독립 트랜잭션으로
처리해 한 시즌의 실패가 다음 시즌을 막지 않는다. 스케줄 실행은 실패한 시즌 수를 보고하지만
UUID를 메트릭 레이블로 만들지 않는다.

## 7. 커밋 뒤 전달

- BRIEF 전용 아웃박스는 WATCH·이메일 아웃박스의 테이블이나 상태를 재사용하지 않는다.
- 원본 트랜잭션은 BRIEF HTTP 호출을 기다리지 않는다.
- 작업자는 커밋 뒤 최소 한 번 전달하며 재시도마다 같은 `eventId`와 의미상 같은 본문을
  보낸다.
- 한 번에 한 건을 1분 동안 선점하고, 같은 `signalId`의 후속 리비전은 앞선 리비전이
  `PENDING`·`PROCESSING`인 동안 열지 않는다. 만료된 작업은 새 작업 구분 토큰과 증가한 시도 횟수로
  회수한다.
- BRIEF의 `200`·`202`는 완료, `400`·`409`·`422`는 계약 또는 생산자 데이터 실패,
  `429`·`5xx`·네트워크 실패는 재시도 가능 결과로 분류한다.
- 그 밖의 HTTP 상태도 영구 실패로 기록한다. 재시도 가능 결과는 다음 설정형 스케줄러
  실행에서 다시 선점하며 별도 최대 시도 횟수와 재시도 간격은 정하지 않는다.
...
- 전달은 기본 비활성이다. 로컬에서는 루프백 HTTP 출처를, 그 밖의 환경에서는 HTTPS
  출처만 허용하고 리디렉션을 따르지 않는다. 경로·사용자 정보·쿼리·프래그먼트는
  ...
  Spring 구성 트리의 `baton.brief.bearer-token`으로 마운트한다. 두 방식 모두 BRIEF
...
6. 완료: 실제 두 프로세스 흐름에서 전용 Bearer 인증과 새·직전 토큰 중첩 교체를
   검증했다.
7. 완료: 기존 BATON 프로덕션 Compose에 BRIEF HTTPS 출처, 명시적 재계산 주기와 소유자
   전용 Bearer 파일의 구성 트리 주입 경계를 연결했다.
8. 실제 공개 HTTPS 스테이징에서 전달·재시도·토큰 교체를 검증한다.
...
- 재계산과 재시작 뒤에도 같은 신호 식별 기준이 같은 `signalId`를 사용한다.
- 리비전은 신호별로 연속 증가하고 동일 상태 재계산은 새 행을 만들지 않는다.
- 시간만 지난 발생·해소·심각도 변경을 고정 `Clock`과 시즌 시간대로 재현한다.
- 원본 변경과 같은 트랜잭션에 불변 아웃박스를 기록하고 커밋 뒤 같은 이벤트를 재전달한다.
...
- WATCH·이메일 아웃박스 테이블과 전달 상태 재사용
- 원본 변경 트랜잭션 안의 BRIEF 동기 호출
- 브로커, BATON 사용자 계정 기반 BRIEF 관리 인증·인가, 최대 재시도 횟수·별도 재시도 간격의 임의 채택
- 신호 목록 API, BRIEF 관리 UI와 실패 재처리 UI 구현

## 11. 검증 상태와 남은 작업

BRIEF 커밋 `1e9dd22`의 `2.0.0-rc.4` `VERSION`·JSON Schema·일곱 예시를 BATON 저장소에
...
MySQL 8.4에서 신호에 영향을 주는 원본 변경이 수동 재계산 요청 없이 같은 신호 식별 기준의
최초 `ACTIVE`, 동일 계산 시 변경 없음, `RESOLVED → ACTIVE`, 심각도 변경의 `1..4` 연속 리비전과
안정적인 `signalId`·`sourceReference`를 만드는지 확인했다. 원본 변경과 자동 재계산을 같은
트랜잭션에 두고 롤백했을 때 원본과 아웃박스가 함께 복원되고, 시즌 종료가 다음 리비전의
`RESOLVED`를 기록하는 것도 확인했다. 자동 회차 생성은 저장 직후 같은 시즌 재조정을
한 번 호출한다. 두 역할의 담당자를 동시에 해제해도 두 신호가 모두 최초 `ACTIVE`로
남고 잘못된 `RESOLVED` 이벤트가 생기지 않는지 확인한다. 시간 기준 재계산도 진행 중인 원본
변경의 커밋을 기다린 뒤 같은 신호 상태를 확인하며 불필요한 리비전을 만들지 않는다.
열린 시즌의 기존 상태 일괄 반영, 아직 활성 신호가 남은 종료 시즌의 마지막
...
아웃박스가 같이 커밋되는지 확인했다. BRIEF 파생 신호·아웃박스만 비운 뒤 BATON을 재기동하여
스케줄러가 기존 원본에서 `WARNING` `ROLE_UNASSIGNED` 리비전 1을 처음 생성하는지
...
BRIEF 현재 점검 항목의 리비전 2·3에 반영됐다.

응답 유실은 실제 TCP 응답 절단이 아니라 BRIEF 수신 뒤 BATON의 전달 지속 상태를 같은
재시도 상태로 되돌려 재현했다. 이후 BRIEF 이벤트 수신 인증을 필수화하고 BATON
`RestClient`에 전용 Bearer를 설정한 같은 시나리오도 성공했다. BRIEF의 새 토큰과 직전
토큰 중첩 구간에서 BATON이 직전 토큰을 계속 보내는 순차 배포도 정상 처리됐다.
기존 BATON 프로덕션 Compose와 사전점검은 BRIEF 전달 조건·HTTPS 출처·명시적 재계산
