...
활성 루틴이 없어 커서만 전진한 경우에는 재조정하지 않는다. 신호 의미를 바꾸지 않는 접근 키,
...
시즌 종료·재개, 구성원 활동 상태, 루틴·회차·바통 항목·역할 자료의 보관·복원과 실행·바통 항목의
완료 여부는 저장된 상태가 실제로 바뀔 때만 재조정한다. 같은 상태를 다시 요청해도 기존 권한·소유권,
시즌 잠금과 편집 제한 검사는 그대로 수행한다.
역할 자료의 제목·URL·설명과 바통 항목의 문구·분류 수정은 같은 역할 안에서는 신호 판정 입력을
...
역할·수동 회차·바통 준비·바통 항목·역할 자료 생성은 실제 저장 직후에만 BRIEF를 재조정한다.
...
역할 바통의 전달·수락·취소도 실제 상태 저장 뒤에만 BRIEF를 재조정한다. 같은 확인자 명의로
...
빠진다. 종료 시즌은 신호 계산용 구성원·역할·루틴·회차·실행·바통·자료를 조회하지 않고,
