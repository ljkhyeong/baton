# PRD-0009: BATON 경유 BRIEF 점검 항목 요약과 필터 조회
...
BATON 운영 점검의 현재 결과와 저장된 주간 요약은 각각 따로 제공하며, 두 서비스의 반영 시점은 다를 수 있다.
...
`aggregateRevision`은 ‘원본 버전’, `sourceReference`는 ‘원본 항목 ID’,
`revisionGap`은 ‘기록 누락 이력’으로 표시한다. 누락 이력의 개수는 해당 항목 수이며 누락 건수가 아니다.
API 필드·상태 코드와 판정 기준은 그대로 유지한다.
...
불변 생성본 `ETag`를 재사용하지 않는다. 조회에는 CSRF token을 요구하지 않는다.
...
요약에는 상태·심각도·기록 누락 이력 필터를 적용하지 않는다.
...
업무 완료 수·생성본 `removed` 개수·불변 과거 스냅샷이 아니다.

화면은 미해결 요약과 별도로 주차·시간대·확인 시각과 함께 표시하고 첫 페이지 새로고침 때
다시 읽는다. 건수를 누르면 해결 목록·시점을 펼치며 원본 항목 ID와 해결 시 원본 버전은
‘해결 기록 보기’에 접어 둔다. 현재 업무 이름과 이동은 PRD-0010의 기존 권한 확인·원본 연결을 사용한다.
...
- 요약 버튼으로 활성 심각도 또는 기록 누락 이력 조건의 첫 페이지를 연다.
- 상태·심각도·기록 누락 이력 선택, 다음 페이지와 첫 페이지 새로고침을 제공한다.
...
- 기존 선택 실행 `BriefEditionHttpsEndToEndTest`는 실제 두 JAR·MySQL 8.4·PostgreSQL 18.6·
  서비스 Caddy에서 로그인·멤버십·접근 키, 빈 요약, 심각도·기록 누락 이력 교집합과 필터된 다음 페이지,
