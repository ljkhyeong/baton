...
응답은 BRIEF가 저장한 생성본 식별자, 생성 번호, 집계 기간, 시간대, 원본 cursor, 규칙 버전과
...
최신 생성본은 BRIEF 전체 스냅샷을 반환한다. 항목의 `aggregateRevision`과 `revisionGap`은
이전 BRIEF 생성본에서 근거를 알 수 없으면 `null`일 수 있다.
...
| `editionId` | BRIEF 불변 생성본 UUID |
| `generation` | 작업공간·시즌 범위 생성본 세대 |
...
- PRD-0009의 인증된 ‘주간 업무 점검’ 패널 안에서 ‘저장된 주간 요약’를 펼칠 때 최신 생성본을 조회한다.
- 생성 시점의 주차·시간대·시각·생성 번호와 저장된 항목을 표시한다. 현재 시즌 시간대나 현재 관심
  항목으로 과거 내용을 바꾸지 않으며, 최신 생성본을 이번 주 생성본이라고 추측하지 않는다.
- 저장된 `section`에 따라 이번 주 변경·이월된 미해결 항목를 나눠 표시한다. `null`은 별도의
  이전 주간 요약·분류 미기록 그룹으로 표시한다.
- 이전 항목의 `aggregateRevision`·`revisionGap`이 함께 `null`이면 ‘원본 버전·누락 이력 미기록’으로 표시한다.
- ‘이번 주 요약 생성’은 본문 없이 기존 POST API를 호출한다. 동적 CSRF 헤더는 공용 계정 API에서
  받고 `Origin`·`Sec-Fetch-Site`는 브라우저에 맡긴다. 서버의 주차·전달 경계·실행 기록을 복제하지 않는다.
- 명시적 클릭만 생성하며 자동 재시도하지 않는다. 대기 중에는 중복 제출을 막고 성공하면 최신
  조회를 갱신한다. `created=false`는 기존 주간 요약 재사용으로 표시한다.
- `404 BRIEF_EDITION_NOT_FOUND`, 전달 미완료·진행 중 `409`, 서비스 `503`과 사용자 권한 거부를 구분한다.
  실패하면 최신 주간 요약을 확인하고 필요한 경우 생성 버튼으로 다시 요청하도록 안내한다.
- 종료 시즌은 조회만 허용한다. 권한 거부 때 이전 생성본과 생성 결과를 감추고 조회 성공 뒤에만
  다시 표시한다. 브라우저에 BRIEF 서비스 token이나 별도 주간 요약 저장소를 두지 않는다.
...
- 알 수 없는 신호 유형·상태와 이전 주간 요약의 `null` 메타데이터를 허용한다.
