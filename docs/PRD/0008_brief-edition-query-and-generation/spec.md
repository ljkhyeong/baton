...
- 오늘 화면의 `주간 운영 요약`을 펼칠 때만 최신 조회를 시작한다. 로그인 필요, 구성원 미연결,
  활동 종료와 로그인·구성원 조회 실패를 구분하고 기존 로그인·구성원 관리 흐름으로 연결한다.
- 계정·팀·시즌·공유 접근 키별 React Query 키를 사용한다. 영역을 닫거나 인증 확인에 실패하면
  요약 화면을 내리고, 사용하지 않는 조회 캐시를 보관하지 않는다. 브라우저 저장소에 요약을 쓰지 않는다.
- `BRIEF_EDITION_NOT_FOUND`만 아직 생성한 요약이 없는 상태로 표시한다. 서비스 미설정·연결 실패,
  다른 `404`나 잘못된 응답을 빈 요약으로 취급하지 않는다.
- `이번 주 요약 만들기`는 본문 없이 기존 생성 API를 호출한다. 진행 중에는 중복 클릭을 막고
  POST를 자동 재시도하지 않는다. 성공하면 최신 조회를 갱신하고 새 생성과 기존 결과 재사용을 구분한다.
- 전달 대기·다른 생성 진행 중·연결 설정 오류·일시 실패를 구분한다. 통신 오류로 생성 결과를
  확인하지 못하면 최신 요약을 먼저 다시 조회하도록 안내한다. 서버 오류의 요청 ID를 유지한다.
- 종료 시즌은 기존 요약만 조회한다. 주차는 응답의 `weekStart`, 시각은 응답의 `zoneId`로 표시하며
  오래된 최신 요약을 이번 주 요약으로 바꾸어 부르지 않는다.
- 항목의 신호 유형·심각도·상태·관찰 시각과 식별 정보를 표시한다. 현재 응답에는 과거 역할 이름과
  원본 이동 주소가 없으므로 현재 역할을 추측해서 붙이거나 불투명 참조로 이동 주소를 만들지 않는다.
- 알 수 없는 신호 유형·상태도 폐기하지 않고 원래 값을 확인할 수 있게 한다. 사용하지 않는 개정
  메타데이터는 화면 모델에 복제하지 않으며 기존 요약의 `null` 값도 허용한다.
- 일시적인 최신 조회 실패는 마지막 조회 내용을 안내와 함께 유지한다. 접근 거부 또는 잘못된
  응답은 기존 요약을 숨기고, 재조회가 성공하기 전에 생성 오류의 접근 거부 표시를 지우지 않는다.
