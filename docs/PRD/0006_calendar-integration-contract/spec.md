...
화면의 계정 UUID가 현재 세션 계정과 같은지 먼저 확인한다. 로그인한 계정은 워크스페이스 접근 키를 확인한 뒤 자기 계정·시즌의 구독을 관리한다.
활성 팀 구성원만 현재 시즌의 주소를 발급·재발급할 수 있다. 종료 시즌과 활동이 중지된 계정은
자기 구독 상태 확인과 폐기가 가능하다. 다른 계정의 구독 ID를 입력하는 API는 제공하지 않는다.
...
| 사용자 폐기·구성원 활동 중지 | 폐기 의도를 먼저 기록하고 같은 ID로 CAL DELETE 재전달 |
| 폐기 완료 뒤 다시 구독 | 이전 ID를 재활성화하지 않고 새 구독 ID로 발급 |

구성원 활동 중지는 원본 변경과 같은 트랜잭션에 모든 시즌의 폐기 요청을 남긴다. 원본 롤백은
폐기 요청도 롤백하고, 이후 구성원을 재활성화해도 이미 남긴 폐기 요청은 지우지 않는다.
전달이 켜진 BATON은 기본 5초마다 50건까지 폐기하며, 연결 실패나 진행 중인 발급은 다음 주기에
다시 처리한다. 구독 발급 기능을 끈 뒤에도 기존 폐기는 계속한다. 이전 데이터 보완을 위해 현재
활성 구성원 연결이 없는 구독도 점검한다. CAL이 폐기를 수신하기 전에는 주소가 작동할 수 있고,
