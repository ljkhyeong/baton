...
| 생성 응답 유실 | ID를 유지하고 상태 조회. **구독 없음**이면 같은 ID로 다시 생성 가능 |
...
활성 구성원 연결이 없거나 계정 권한 팀에서 권한이 회수된 구독도 점검한다. CAL의 구독 해제가 완료되기 전에는 기존 주소를 사용할 수 있고,
앱이 이미 내려받은 일정은 앱에서 별도로 제거해야 한다.

오늘 화면의 **내 캘린더에 추가**에서 구독 상태·주소 발급·복사·재발급·해제를 제공한다.
`ACTIVE`는 **구독 주소를 사용할 수 있습니다.**, 목록에서는 **주소 사용 가능**으로 표시한다.
캘린더 앱 등록 여부는 확인하지 않으며, `NOT_CREATED`는 **발급된 구독 주소가 없습니다.**로 안내한다.
계정 화면 `/account`의 **내 캘린더 구독**에서 본인 소유의 팀·시즌별 구독 기록을 20개씩 조회한다.
권한을 잃은 팀과 해제한 구독도 포함하며, 현재 팀·시즌 이름과 BATON 관리 상태만 제공한다.
팀·시즌 이름 검색은 아직 불러오지 않은 구독까지 서버에서 조회하며, 해제된 구독을 숨길 수 있다.
검색·필터를 바꾸면 첫 페이지부터 조회하고 해제 처리 중인 기록은 숨기지 않는다.
목록에서 항목을 펼칠 때 최신 구독 상태를 확인하고 개별 구독을 해제한다. 확인한 상태와 시각은
항목을 접어도 현재 계정 화면에 남기며 계정·구독 ID가 바뀌거나 항목이 제거되면 버린다.
확인 시각은 브라우저가 구독 상태를 받은 시각이며 외부 캘린더 앱의 동기화 시각이 아니다. 새 주소 발급은 접근
가능한 팀 화면에서 수행한다. 목록 응답에는 주소가 없으며 계정 변경 시 이전 목록과 펼친 항목을
제거한다. CAL 연결 실패는 목록 전체 조회를 막지 않으며 상세 조회 실패만 표시한다.
팀 접근이 거부된 화면에서도 로그인한 소유자에게 **내 캘린더 구독 해제**를 제공한다.

**여러 구독 선택**에서는 불러온 항목 중 해제되지 않은 구독을 최대 20개 선택한다. 더 보기로
불러온 페이지 사이의 선택은 유지하고 검색·필터·계정 변경 때는 지운다. 전체 선택은 불러온
항목에만 적용하며 20개를 넘으면 앞의 20개만 선택한다. 확인 화면에는 실행할 팀·시즌 목록을
고정해서 표시하고 확인·실행 중에는 선택·검색·필터·더 보기·목록 새로고침을 잠근다.
선택 모드에서는 개별 관리 화면을 접고 **선택 마치기**로 돌아오면 다시 상태를 확인할 수 있다.

일괄 해제는 기존 소유자 상태 GET과 DELETE를 항목별로 차례로 호출하며 계정 일치 헤더와
CSRF 검증을 유지한다. 각 항목의 사전 조회에서 구독이 없거나 이미 해제되었으면 완료로 표시하고
DELETE를 생략한다. 구독 ID가 선택 당시와 다르면 해제하지 않고 확인이 필요한 항목으로 남긴다.
해제 응답을 확인하지 못하면 상태를 한 번 조회해 **해제됨**, **해제 처리 중**, **해제 여부 확인 필요**로
구분하며 DELETE를 자동 반복하지 않는다. 한 항목의 연결 실패는 나머지 항목의 처리를 막지 않는다.
인증·계정 확인 실패, 계정 변경, 화면 이탈이나 **남은 구독 해제 중단**은 이후 요청을 중단한다.
이미 보낸 해제 요청은 서버에서 완료될 수 있으므로 중단을 해제 취소로 안내하지 않는다.
결과 개수는 해제됨·처리 중·확인 필요로 나누고, 요청하지 않은 항목이 있으면 **요청 안 함**으로 따로 집계한다.
항목별 상태를 표시하고 마지막에 현재 검색·필터 조건으로 목록을 갱신한다. 처리 중이거나 확인이 필요한
항목은 선택을 마친 뒤 기존 개별 화면에서 다시 확인하고, 요청하지 않은 항목은 다시 선택해 해제한다.
선택·확인 목록과 결과는 현재 화면에만 두며 구독 주소나 토큰을 추가로 보관하지 않는다.

열린 항목이 `IN_PROGRESS`·`REVOCATION_PENDING`이면 상태 GET만 3초 간격으로 최대 90초간
자동 조회한다. 완료·조회 실패·항목 닫기 때 중단하며 배경 탭에서는 자동 조회하지 않는다.
90초가 지나면 자동 확인을 멈추고 **상태 다시 확인**을 안내한다. 조회 실패 때는 중단 사유를 표시한다.
상태 다시 확인을 누르면 종료 안내를 지우고 제한 시간을 새로 시작한다. 발급·재발급·해제 요청 자체는 자동 반복하지 않는다.
개별 구독 오류는 서버·공용 클라이언트의 안내를 표시하고 같은 재확인 문장을 덧붙이지 않는다.
해제 응답 유실 뒤 상태 조회로 `REVOKED`를 확인하면 이전 해제 오류를 더 표시하지 않는다.
해제 완료를 확인하면 현재 검색·포함 조건으로 목록을 갱신해 해제된 구독 숨기기를 즉시 적용한다.
주소는 열린 화면의 임시 상태에만 두며 조회 캐시, 변경 요청의 반환 캐시와 브라우저 저장소에
넣지 않는다. 패널을 닫거나 계정·시즌·접근 키·활성 자격이 바뀌면 제거한다. 새 주소 발급과
해제는 기존 주소에 미치는 영향을 보여준 뒤 실행한다. 기존 주소가 있으면 그대로 등록하고, 주소를
분실하면 재발급하도록 안내한다. 화면에서 주소를 숨기는 동작을 구독 해제로 표현하지 않는다.
Google·Apple·Outlook의 URL 구독 절차와 갱신 지연, 파일 가져오기와 URL 구독의 차이를 함께 안내한다.
주소 발급 후 등록 안내를 자동으로 펼치고 선택한 앱의 절차만 표시한다. Google은 공식 URL 등록 화면,
Outlook은 개인·회사/학교 계정별 캘린더, Apple은 공식 등록 안내로 연결한다. 외부 링크에는 구독 주소나
접근 키를 넣지 않고 `noreferrer`로 참조 주소 전달을 막는다. 주소 복사·앱 선택은 CAL API를 재호출하지 않는다.
...
화면 검증은 `cd frontend && npm run e2e -- calendar-subscription`이며 PC·모바일·WebKit에서
주소 임시 보관, 응답 유실 후 수동 복구, 비로그인 차단과 잘못된 주소 거부를 확인한다.
계정 목록은 다른 계정 응답 차단, 페이지 이동, 조회 실패 재시도와 접근 권한 없는 본인 해제를 검증한다.
서버 검색·해제 필터와 페이지 초기화, 상태·확인 시각 유지, 자동 조회 완료·제한 시간·실패·닫기 중단,
해제 응답 유실 뒤 조회만으로 완료 확인도 검증한다.
...
- 같은 원본의 시각이 직전 시각보다 늦지 않으면 원본 행 잠금 안에서 직전 시각에 1마이크로초를 더한다.
