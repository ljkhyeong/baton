...
- 상태: 생산자 직렬화·트랜잭셔널 아웃박스·HTTP 전달 구현, 기존 데이터 조정·실제 CAL 검증 미완료
- 기준일: 2026-08-25
...
`BATON_CAL_CAPTURE_ENABLED`와 `BATON_CAL_DELIVERY_ENABLED`의 기본값은 `false`다. 전달을 켤 때는
...
기존 데이터 조정과 실제 CAL 검증은 아직 완료하지 않았으므로 운영에서는 두 기능을 켜지 않는다.
...
- 실제 CAL rc.2 컨테이너를 대상으로 생성·변경·취소, 응답 유실 뒤 `DUPLICATE`와 순서 뒤바뀜을
  검증한다.
