...
- 상태: 생산자 직렬화·트랜잭셔널 아웃박스 구현, HTTP 전달 미구현
- 기준일: 2026-08-25
...
취소 전달 근거가 남아야 하기 때문이다. `BATON_CAL_CAPTURE_ENABLED`의 기본값은 `false`다. HTTP
전달 작업자가 없는 현재 단계에서는 운영에서 이 값을 켜지 않는다. 인증, 재시도와 기존 데이터
조정은 아직 구현하지 않았고 현재 코드는 CAL에 네트워크 요청을 보내지 않는다.
