...
| `roles` | 현재 시즌의 역할 스냅샷, 담당자·기간, 책임과 업무 주의사항 목록 |
...
- 역할에 업무 주의사항이 있으면서 책임 목록이 없거나, 활성 바통 항목이 없거나 미완료이거나, 역할 자료가 없으면 사용자에게 기록한 위험과 부족한 준비 요소를 한 신호의 이유에 함께 설명한다.
...
열린 역할 바통이 `PREPARING`이면 현재·다음 담당자와 담당 기간은 바통 준비 시점 값으로 고정하지만 역할의 이름·목적·책임·업무 주의사항은 수정할 수 있다. `TRANSFERRED`이면 수락 또는 취소 전까지 역할 전체를 수정할 수 없다. 위반은 `409 ROLE_HANDOFF_STATE_CONFLICT`다.
