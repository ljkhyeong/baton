...
팀 초대와 팀 단위 권한은 후속 [PRD-0009](../0009_team-account-access/spec.md)가 소유한다.
...
로그인 화면은 로그인 방법과 별도 계정 처리만 안내한다. 공급자 토큰 저장 방식과 향후 계정 연결 계획은 이 문서에서 관리하며 로그인 화면에는 표시하지 않는다.
...
- OAuth 공급자의 접근·갱신 토큰, Google ID 토큰과 사용자 정보 클레임은 신원 해석
  중에만 유지한다. 최초 세션 저장 전 계정 UUID·로그인 당시 세션 버전과 `ROLE_ACCOUNT`만 가진 인증 주체로
...
자체 이메일 로그인이 성공하면 진행 중인 세션 조회를 취소하고 새 세션을 조회해 로그인 결과를 확인한다.
로그인 전에 시작한 미인증 조회가 늦어도 해당 결과를 기다리거나 로그인 실패로 처리하지 않는다. 로그인 요청 뒤 회원가입이나 다른 화면으로 이동하면 새 세션 조회와 캐시 갱신은 완료하되 기존 복귀 경로로 자동 이동하지 않는다.
...
인증 성공 뒤 응답만 유실되면 같은 토큰의 재시도는 이미 사용된 토큰으로 거부된다. 검증 토큰 오류
화면은 이 가능성을 안내하고, 비밀번호를 정한 뒤 결과를 확인하지 못한 사용자가 해당 비밀번호로
로그인할 수 있는 경로를 제공한다. 아직 인증하지 않은 사용자는 새 인증 메일을 요청할 수 있다.
이미 인증된 계정에는 새 검증 메일을 보내지 않으며, 이 복구 안내로 일회용 토큰 규칙이나 계정 존재
여부를 구분하지 않는 응답 계약을 바꾸지 않는다.
...
- `/forgot-password`에서 인증된 자체 이메일 계정의 재설정 메일을 요청한다. 존재하지 않는 이메일,
  미인증 계정과 소셜 신원에도 같은 접수 안내를 표시하며 계정·자격 증명을 새로 만들지 않는다.
- 요청 시각부터 30분 동안 유효한 일회용 토큰을 발급한다. 현재 유효한 재설정 토큰이 있으면
  그대로 유지하고, 만료되거나 소비된 뒤 새 요청을 받으면 교체한다.
- 가입용과 재설정용 토큰은 용도와 해시 구분값을 분리한다. 기존 암호화 메일 아웃박스,
  발송 재시도·임대·만료 처리와 요청률 제한을 공유한다.
- 링크의 `/reset-password#token=...` 프래그먼트는 화면에서 읽은 직후 주소창에서 제거하고
  브라우저 저장소에 저장하지 않는다. 새 비밀번호와 확인값은 기존 가입 인증 양식을 공유한다.
  같은 탭에서 다른 메일 링크를 열면 새 토큰의 입력 화면으로 교체하고 이전 토큰의 늦은 결과를
  새 화면에 표시하지 않는다.
- 토큰 소비, 비밀번호 저장과 계정 세션 버전 증가는 하나의 트랜잭션이다. 기존 모든 계정 세션은
  다음 요청에서 무효화하며 자동 로그인하지 않는다. 이미 시작한 요청은 취소하지 않는다.
  팀 공유 접근 키와 발급된 ROUND 참여권은 그대로 유지한다.
- 로그인은 비밀번호 해시와 세션 버전을 같은 DB 조회에서 읽는다. 변경 전에 시작한 로그인도
  이전 버전으로 세션을 만들므로 이후 요청에서 거부한다. 로그인 중 해시 알고리즘 갱신은
  처음 읽은 해시가 여전히 같은 경우에만 저장해 재설정한 비밀번호를 덮어쓰지 않는다.
- 세션 버전 확인 중 DB 장애는 세션을 지우지 않고 `503`으로 응답한다.
- 제출을 자동 재시도하지 않는다. 응답을 받지 못했거나 사용한 토큰으로 거부되면 입력한 새
  비밀번호로 먼저 로그인하거나 새 재설정 메일을 요청하도록 안내한다.
...
- `/account`는 로그인한 계정의 표시 이름과 연결된 로그인 방법을 보여 준다. 공급자 식별자는
...
- 공개 계정 연결 엔드포인트는 최근 재인증과 연결 의도 수명·감사 계약을 구현할 때까지 제공하지 않는다.
- 마지막 로그인 방법 제거와 계정 병합은 첫 버전 범위가 아니다.
...
PRD-0009의 계정 전환 팀은 공유 키 연결 진입점을 닫고 기존 멤버십은 승인 전 상태로 보존한다. 명시적으로 권한을 받은 활성 구성원만 작업 공간에 접근하고 새 ROUND 참여권을 발급받는다.
...
구성원 연결에 성공하면 해당 계정·팀·접근 키 범위의 진행 중 조회를 취소한 뒤 연결 결과를 반영한다.
연결 전에 시작한 미연결 응답이 늦게 도착해도 완료 상태를 덮어쓰지 않는다.
...
- `login_failed`: `소셜 로그인을 완료하지 못했습니다.` / `다시 시도하거나 다른 로그인 방법을 선택해 주세요.`
...
ROUND 시작 요청 뒤 해당 화면을 떠나면 늦은 응답으로 입장 정보를 저장하거나 방으로 자동
이동하지 않는다. 다시 돌아오면 서버 목록에서 이미 만든 방을 확인하고 사용자가 입장을 선택한다.
