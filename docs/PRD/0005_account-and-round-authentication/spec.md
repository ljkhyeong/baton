...
현재 room mapping 목록 조회는 팀 접근 키와 활동 중인 membership을 한 번 확인한 뒤 team·season의
서버 영속 매핑을 한 번에 조회해 권위로 반환한다. 브라우저 sessionStorage는 ROUND 입장과 복귀를
돕는 힌트이며 매핑의
존재·종료 여부를 결정하지 않는다.
