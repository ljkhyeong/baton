...
- 상태: 양방향 1차 연동 계약·수신 구현, 공개 스테이징 검증과 상태 프로젝션·표시 미완료
- 기준일: 2026-08-02
...
- WATCH의 직접 HTTPS 상태 변경 이벤트 송신자와 BATON의 내구성 있는 수신기는 저장소에 구현했지만 실제 공개 스테이징의 WATCH→BATON 전달, 응답 유실 뒤 같은 `eventId` 재전송, 적체 해소와 운영 활성화는 아직 검증하지 않았다. 이 검증은 상태 프로젝션·UI 착수 전 관문이며 이벤트 브로커가 있다고 가정하지 않는다.
