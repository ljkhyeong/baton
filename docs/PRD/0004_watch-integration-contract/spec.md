...
- 최근 연결 판정 `lastConclusiveAt`이 없으면 `UNKNOWN/PENDING`, 판정이 정확히 5분
  ...
  화면은 최근 연결 판정과 최근 점검 시도를 구분해 표시한다. 현재 원본과 일치하는 응답의
...
- 프런트는 온라인 여부·조회 일시 중지·캐시의 30초 `staleTime`을 확인한다. 오프라인이나
  ...
  캐시가 만료되면 `최근 상태 다시 조회 필요`, 재조회 중이면 `연결 상태 확인 중`, 오프라인이면
...
- 열린 역할 상세는 탭 복귀·네트워크 재연결 때 항상 GET을 다시 요청한다. 사용자는
  `상태 다시 조회` 버튼으로도 같은 GET을 보낼 수 있다. 기존 조회 분산·취소·시간 제한을
...
- 바통 전달이나 수정 충돌 복구 대기로 편집이 잠겨도 상태 조회는 유지한다. 화면의 편집 제한을
  ...
  편집 제한 중에는 기존대로 다시 점검 버튼을 표시하지 않지만 `상태 다시 조회`는 사용할 수 있다.
...
- 역할 상세의 상태 배지·최근 연결 판정 및 점검 시도 시각·수동 재점검은 구현되어 있다. 푸시 알림과 상태 이력 UI는 제공하지 않는다.
