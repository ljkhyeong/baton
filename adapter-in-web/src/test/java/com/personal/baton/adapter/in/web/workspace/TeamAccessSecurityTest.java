...
import com.personal.baton.application.workspace.port.in.InspectResourceHealthUseCase.CheckStatus;
...
@WebMvcTest({TeamAccessController.class, WorkspaceRecordsController.class, WorkspaceResourceHealthController.class})
...
class TeamAccessSecurityTest {
    ...
    @MockitoBean InspectResourceHealthUseCase resourceHealth;

    @Test
    @DisplayName("로그인한 자료 재점검은 계정 확인 헤더와 CSRF를 요구한다")
    void bindsResourceCheckToExpectedAccount() throws Exception {
        ...
        String path = "/api/v1/teams/{teamId}/seasons/{seasonId}/role-resources/{resourceId}/check-requests";
        mvc.perform(post(path, ID, ID, ID).with(authentication(auth)).with(csrf())
        ...
        verifyNoInteractions(resourceHealth);
        when(resourceHealth.requestCheck(ID, ID, ID, null)).thenReturn(new CheckResult(ID, CheckStatus.SCHEDULED));
        mvc.perform(post(path, ID, ID, ID).with(authentication(auth)).with(csrf())
                        .header("X-Baton-Account-Id", ID).header("Origin", "http://localhost")
                        .header("Sec-Fetch-Site", "same-origin"))
                .andExpect(status().isAccepted()).andExpect(jsonPath("$.status").value("SCHEDULED"));
        verify(resourceHealth).requestCheck(ID, ID, ID, null);
    }
