...
class ResourceHealthRestDocsTest {
    ...
    private static final String READ_DESCRIPTION = "팀 읽기 권한과 자료 소유권을 확인한 뒤 자료 화면과 별도로 현재 연결 상태를 조회한다.";
    private static final String CHECK_DESCRIPTION = "팀 변경 권한과 자료 소유권을 확인한 뒤 비동기 URL 재점검을 접수한다. 완료 결과가 아니다.";
    ...
    void readHealth(Availability availability) throws Exception {
        ...
        mvc.perform(get(PATH + "/health", TEAM, SEASON, RESOURCE).header("X-Baton-Access-Key", KEY))
                ...
                .andDo(MockMvcRestDocumentationWrapper.document("inspectResourceHealth", READ_DESCRIPTION, "자료 연결 상태 조회",
                        ...
                        requestHeaders(headerWithName("X-Baton-Access-Key").description("공유 키 팀의 접근 키. 계정 권한 팀은 로그인 세션을 사용한다.").optional()),
