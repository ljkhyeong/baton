...
docker run --rm --network none --read-only --tmpfs /tmp \
  ...
  --entrypoint /bin/promtool "$prometheus_image" test rules integration-alerts.test.yml http-alerts.test.yml https-alerts.test.yml host-alerts.test.yml
docker run --rm --network none --read-only \
...
for target in 'alertmanager.yml.example operations-email monitoring-heartbeat-disabled' \
  ...
  for alert_name in BatonMonitoringWatchdog BatonMetricsUnavailable BatonEmailDeliveryRejected BatonApiServerErrorsHigh BatonTlsCertificateExpiring BatonHostDiskSpaceLow; do
