...
cal_bearer_token="$(
  read_secret_or_placeholder "$cal_bearer_token_file" disabled-cal-bearer-token
)"
smtp_password="$(
...
env \
  ...
  -u BATON_CAL_CAPTURE_ENABLED \
  -u BATON_CAL_BACKFILL_ENABLED \
  -u BATON_CAL_RECOVERY_PREPARATION_ENABLED \
  -u BATON_CAL_RECOVERY_RUN_ID \
  -u BATON_CAL_DELIVERY_ENABLED \
  -u BATON_CAL_SEASON_METADATA_ENABLED \
  -u BATON_CAL_SEASON_METADATA_MAINTENANCE \
  ...
  -u BATON_AUTH_PASSWORD_RESET_ENABLED \
