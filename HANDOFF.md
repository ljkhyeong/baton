...
- 다음 단계는 기존 시즌 이름 보정과 복원 후 최신 이름 재전달이다. V29 이름 아웃박스에 기록된
  최신 개정 번호를 보존하고, 이름 연동 설정을 끈 기간의 원본 변경도 보정해야 한다.
- 정식 채택 전 CAL 후보를 불변 사전 릴리스로 게시하고 검증된 자산으로 계약 핀을 갱신해야 한다.
  현재 안정 핀은 `1.0.0`이며 `BATON_CAL_SEASON_METADATA_ENABLED=false`를 유지한다. 기존 시즌·
  복원 절차와 CAL V7 배포 확인 전에 운영에서 켜지 않는다. 실제 캘린더 앱 이름 갱신도 남아 있다.
