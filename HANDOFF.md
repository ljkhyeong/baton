...
- `codex/baton-go-integration`에서 역할 자료 클릭 시 workspace 접근을 확인한 뒤 canonical
  ROUND room만 BATON GO의 15분 만료 short URL로 여는 세로 흐름을 구현했다.
...
- 프런트는 같은 클릭의 실패 재시도에 UUID와 만료 시각을 재사용한다. 다음 경계는
  ROUND browser의 BATON 인증형 room-aware signaling/TURN endpoint 연결이며, GO URL에는
  이후에도 BATON 접근 키나 ROUND join ticket을 넣지 않는다.
