...
- [PRD-0010](docs/PRD/0010_brief-navigation-and-readiness/spec.md)에 따라 현재 업무명·역할/루틴 이동,
  저장된 브리프 이력·비교와 생성 준비 상태를 구현했다.
- 현재 업무 정보는 별도 BATON 조회이며 BRIEF 불변 본문·ETag·이벤트 계약을 유지한다.
- 백엔드 `build checkApiContract`와 실제 두 JAR·MySQL 8.4·PostgreSQL 18.6·서비스 Caddy의
  `BriefEditionHttpsEndToEndTest`가 성공했다. 과거 단건·이력·비교, 다른 작업공간 접근 거부,
  실제 BATON 역할 API로 만든 신호의 업무 연결과 전달 대기 상태를 확인했다. 에디션 비교용
  BRIEF 투영은 격리한 DB에 준비했으며 이벤트 생산·전달 테스트 전체를 재실행하지 않았다.
- 첫 전체 검증에서는 새 통합 테스트가 남긴 열린 시즌 때문에 기존 정합화 테스트 한 건이
  실패했다. 독립 테스트를 기존 원자성 시나리오에 합쳐 정리하고 해당 MySQL 클래스 21건을
  다시 통과시켰다. 이 수정 후 애플리케이션 전체 테스트는 반복하지 않았으며, 마지막 빌드의
  웹 118건·REST Docs 147건·부트스트랩 66건이 성공했다.
- 프런트 빌드와 Chromium·390px 모바일·WebKit의 BRIEF 브라우저 21건이 통과했다.
  전달·확인 시각을 현재 시즌 시간대로 맞춘 뒤 프런트 빌드와 해당 브라우저 시나리오를
  세 환경에서 다시 확인했다. 브라우저는 API 대역을 사용했고 실제 스테이징 검증은 남아 있다.
