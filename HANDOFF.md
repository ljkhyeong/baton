...
- 요약·필터·상태 전이 API와 ‘오늘’ 패널, 최신 불변 에디션 조회·생성 화면을 PRD-0008·0009에
  연결했다. 실제 스테이징의 서비스 인증서·비밀과 비공개 네트워크를 준비한 뒤 활성화하고,
  로그인한 브라우저에서 요약·필터·상태 전이와 생성·재사용을 확인해야 한다.
  로컬 교차 서비스 테스트와 API 대역 브라우저 테스트를 실제 배포 완료로 해석하지 않는다.
- 이벤트 계약 팩과 실제 record 직렬화 테스트는 `2.0.0-rc.4`로 고정했다. 원본 이벤트 모델과
  송신기·outbox는 유지했으며, 공인 HTTPS 스테이징 검증 전에는 안정 버전으로 승격하지 않는다.
- 최종 로컬 검증은 백엔드 `build checkApiContract`, 실제 두 JAR의 전달·HTTPS 조회 테스트,
  프런트 빌드와 BRIEF 대상 브라우저 테스트 12건이 통과했다. 전체 스모크 66건 중 63건은
  처음 통과했고, 작업공간 로딩 시간 초과가 난 기존 3건도 시간 제한 변경 없이 분리 재실행해 통과했다.
