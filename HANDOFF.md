...
- CAL 후보 `1.1.0-rc.1`의 이름 전달 클라이언트와 로컬 교차 서비스 검증 다음 단계는 시즌 생성·이름
  변경의 전용 개정 번호와 아웃박스 기록, 커밋 후 전달, 기존 시즌 보정과 복원 재전달 연결이다.
- 정식 채택 전 CAL 후보를 불변 사전 릴리스로 게시하고 검증된 자산으로 계약 핀을 갱신해야 한다.
  현재 안정 핀은 `1.0.0`이며 시즌 이름은 자동 전달되지 않는다. 실제 캘린더 앱의 이름 갱신도 남아 있다.
