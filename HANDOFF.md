...
- 기존 활성 회차와 마감은 100개 UUID 키셋 페이지와 회차별 짧은 트랜잭션으로 보정한다. 최신
  ...
  수렴한다. `BATON_CAL_BACKFILL_ENABLED`는 기본 `false`이며 한 번 완료한 뒤 다시 닫는다.
- 실제 CAL rc.2 PostgreSQL 컨테이너와 BATON 운영 클라이언트를 연결한 생성·변경·취소·응답 유실
  `DUPLICATE`·역순 `STALE` 검증이 통과했다. 다음 운영 작업은 기존 문자열 적합성 점검, 캡처와
  보정의 선행 활성화, 보정 완료 뒤 전달 활성화와 실제 시즌 피드 확인이다.
