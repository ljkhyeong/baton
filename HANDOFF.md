...
- 로컬 교차서비스 검증은 BATON 실제 signer·JWK와 ROUND runtime 사이의 정상 TURN·WebSocket 수락, 다른 room·issuer·audience·`kid`와 만료 참여권 거부, key 선게시·새 `kid` 즉시 재조회와 이전 key overlap을 고정한다. ROUND 고정 Clock 테스트는 future `iat`의 `+60초` 허용·`+61초` 거부를, Nimbus source 테스트는 60초 cache와 30초 window당 최대 두 번의 JWK source 접근 제한을 고정한다. 첫 unknown `kid`의 cache-miss refresh 1회 뒤에는 서로 다른 unknown `kid`를 연속 제시해도 추가 JWK 조회 없이 모두 `401`로 거부한다. public HTTPS edge에서 AccountMembership claim부터 room mapping·participation refresh를 거쳐 같은 Account `sub`가 TURN·WebSocket 입장까지 이어지는 전체 흐름과 실제 배포 key 회전 리허설은 아직 확인하지 않았다.
