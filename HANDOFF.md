...
- 로컬에서는 `e2e:fullstack`이 실제 local session → AccountMembership claim → authoritative room mapping → participation refresh와 cookie·JWK 서명의 producer 경계를, 교차서비스 테스트가 BATON signer·JWK → ROUND TURN·WebSocket의 consumer 경계를 각각 고정한다. 별도 `e2e:round-edge`도 private CA의 test-only Caddy와 기존 ROUND images를 연결해 같은 브라우저의 Secure cookie로 TURN credential을 발급받고 WSS room에 입장하는 흐름을 고정했다. production Caddy·Compose에는 digest-pinned ROUND runtime, exact participation-cookie proxy, pre-auth rate limit과 restore/lifecycle 상호 배제를 opt-in으로 반영했다. 다음 과제는 실제 release digest와 외부 coturn을 public HTTPS staging에 배포해 UDP·TCP·TLS allocation/media relay, 실제 OAuth Account의 동일 `sub` 입장과 key overlap 회전을 검증하는 것이다.
