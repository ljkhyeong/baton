...
- 로컬에서는 `e2e:fullstack`이 실제 local session → AccountMembership claim → authoritative room mapping → participation refresh와 cookie·JWK 서명의 producer 경계를, 교차서비스 테스트가 BATON signer·JWK → ROUND TURN·WebSocket의 consumer 경계를 각각 고정한다. 별도 `e2e:round-edge`도 private CA의 test-only Caddy와 기존 ROUND images를 연결해 같은 브라우저의 Secure cookie로 TURN credential을 발급받고 WSS room에 입장하는 흐름을 고정했다. 다음 과제는 구현한 route·credential 최소 전달 정책을 production Caddy·Compose에 반영하고, public HTTPS staging에서 실제 coturn relay와 배포 key 회전을 검증하는 것이다.
