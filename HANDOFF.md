...
- 팀을 전환하기 전에 공개 HTTPS에서 운영자 복구 키로 첫 관리자를 지정하고, 두 계정·두 기기로 초대 수락·권한 회수·기존 공유 링크 거부·본인 명의 바통 확인을 검증한다. 기존 팀은 자동 전환하지 않는다.
...
- 공개 HTTPS에서 오늘의 재확인 목록, 여러 팀·진행 중 시즌의 내 업무와 권한 회수 반영, 스터디·팀 운영 시작 구성을 확인한다. 결정·자료·바통 본문의 초안은 같은 브라우저 탭에서 새로고침해 복구하고 계정 전환 시 섞이지 않는지 확인한다.
- 테스트용 계정의 비활성화에서 마지막 관리자 거부, 모든 기존 세션 거부, 로컬·소셜 재로그인 차단과 CAL 개인 구독의 실제 외부 해지 완료를 확인한다. 공유 키와 조직 기록은 보존한다.

## 파일럿 관찰

- 첫 그룹 스터디에서 ‘확인이 필요한 업무’에서 놓칠 뻔한 책임이나 인수인계 공백을 한 번 이상 미리 발견하는지 확인하고 오탐과 행동 문구를 기록한다.
