...
- `codex/manager-integration-identity-20260730`에서 역할 바통 `V13`, BATON GO·ROUND 역할
  ...
  합쳤고, Google OIDC·owner bootstrap·Spring Session 기반을 `V15`로 추가했다.
...
- 로그인 사용자는 `POST /api/v1/identity/invitations/accept`에 session CSRF와 원문 token을
  보내 기존 roster 구성원과 결속하고 팀의 유일한 `OWNER`가 된다. token은 단일 사용이며
  같은 account의 응답 유실 재생만 현재 결속을 반환한다. DB에는 invitation token과 멱등
  키의 SHA-256 hash만 저장한다. application에는 폐기 동작이 있지만 revoke HTTP endpoint는
  이번 범위에서 공개하지 않았다.
...
- ROUND의 BATON room-aware signaling/TURN endpoint와 자동 signaling 재연결 전 참여권 갱신
  ...
  보류한다.
- 다음 제품 우선순위는 로그인·owner bootstrap을 실제로 사용할 프런트 흐름과 owner가
  발급하는 일반 구성원 invitation이다. 그 뒤 session principal과 활성 구성원 결속으로
  BATON ROUND grant를 발급하고 `grant → TURN → WSS`, 만료 갱신과 재연결을 실제 브라우저로
  검증한다.
