...
- BATON CAL 불변 `contracts-v1.0.0-rc.2`와 자산·스키마 SHA-256을 `contracts/baton-cal`에 고정했다.
...
- V23은 CAL 아웃박스에 1분 임대와 fencing token을 추가했다. 전용 작업자는 원본별 이전 미종결
  ...
  `429`·`5xx`는 최대 1시간의 지수 backoff로 재시도하며 계약 `4xx`는 현재 행을 실패로 끝낸다.
- `BATON_CAL_CAPTURE_ENABLED`와 `BATON_CAL_DELIVERY_ENABLED`는 모두 기본 `false`다. 전달을 켜려면
  경로가 없는 HTTPS 출처, 별도 Bearer와 합계 45초 이하의 연결·읽기 시간 제한을 설정해야 한다.
  다음 작업은 기존 활성 회차·마감 조정과 실제 CAL rc.2 컨테이너를 잇는 생성·변경·취소·응답 유실
  검증이다. 이 검증 전에는 운영에서 캡처와 전달을 켜지 않는다.
