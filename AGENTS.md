...
- Java, Spring과 현재 의존 라이브러리가 제공하는 표준 API로 충분하면 같은 기능의 래퍼, 포트, 어댑터나 검증기를 새로 만들지 않는다. 별도 추상화는 도메인 의미, 교체 가능한 외부 경계 또는 독립된 정책을 실제로 소유할 때만 둔다.
- 같은 소유자가 같은 조건과 실패 시점을 보장할 때만 중복 검증을 제거한다. 입력 형태, 권한·소유권, 도메인 불변식과 DB 제약의 책임을 구분하고, 외부 입력·인증·DB 제약·잠금·부작용 직전 방어처럼 독립 실패 경계는 보존한다.
...
- 응답 디코더는 신뢰 경계의 형태·기본 타입·열거형·식별자 형식과 요청·부모·캐시·동작 대상의 결합을 확인한다. 서버가 소유하는 상태 전이와 파생 상태는 다시 계산하지 않되, 렌더링 안전성과 잘못된 대상 조작을 막는 시간대·날짜 범위·계보·포함 관계·중첩 식별자 상관관계는 보존한다.
- 지원 범위가 맞으면 브라우저와 React가 제공하는 표준 수명주기·접근성·저장소 API를 우선하고, 같은 동작을 수동으로 재구현하지 않는다.
...
- JDK, Spring, Flyway나 사용 라이브러리 자체의 동작과 난수 충돌 확률을 다시 시험하지 않는다. 프로젝트가 선택한 설정, 실제 마이그레이션 결과와 BATON 고유 계약만 검증한다.
- `Flyway.migrate()` 성공 뒤 `flyway_schema_history.success`를 다시 조회하지 않고, 이관된 데이터·제약·인덱스 같은 실제 사후조건을 검증한다.
- 같은 HTTP 계약을 REST Docs가 이미 검증하면 별도 컨트롤러 테스트를 복제하지 않고, 고유한 요청 매핑·보안·쿠키 증거만 기존 계약 테스트에 합친다. 제거된 분기, 프레임워크 자체 동작과 커버리지 수치만을 위한 테스트는 만들지 않는다.
...
- 새로 작성하거나 수정하는 사람 대상 문서는 한국어로 작성한다. 기존 문서에서 일반 영문 산문을 발견하면 같은 변경에서 한국어로 바로잡는다.
- 파일·디렉터리 경로, URL, API 경로, 환경 변수, 명령, 코드 식별자, JSON 필드, 헤더, 열거형 값과 표준·제품·라이브러리·프로토콜 이름은 원형을 보존하고, 그 의미를 설명하는 문장은 한국어로 작성한다.
