...
test('BRIEF 요약에서 조건을 선택하고 다음 페이지와 필터 초기화를 확인한다 @smoke', async ({ page }) => {
  ...
  await panel.getByRole('button', { name: '변경 이력 보기' }).click()
  const history = panel.getByRole('region', { name: '점검 항목 변경 이력' })
  await expect(history.getByText('미해결', { exact: true })).toBeVisible()
  await expect(history.getByText('BATON 판정 심각도: 주의')).toBeVisible()
  expect(historyCalls.at(-1)?.get('sourceReference')).toBe('role:+& 한글')
  await history.getByRole('button', { name: '이전 변경 이력' }).click()
  await expect(history.getByText('해결', { exact: true })).toBeVisible()
  await expect(history.getByText('BATON 판정 심각도: 긴급')).toBeVisible()
  await history.getByText('변경 근거 보기', { exact: true }).click()
  await expect(history.getByText('원본 버전 2', { exact: true })).toBeVisible()
  await expect(history.getByText('이 변경에서 기록 누락 발견', { exact: true })).toBeVisible()
  expect(historyCalls.at(-1)?.get('beforeAggregateRevision')).toBe('3')
  await history.getByRole('button', { name: '최신 이력부터 새로고침' }).click()
  await expect(history.getByText('미해결', { exact: true })).toBeVisible()
  await panel.getByRole('button', { name: '높은 심각도 2건' }).click()
  await expect(history).toHaveCount(0)
  await panel.getByRole('combobox', { name: '기록 누락 이력', exact: true }).selectOption('false')
  ...
  await panel.getByRole('button', { name: '기록 누락 이력 1건' }).click()
