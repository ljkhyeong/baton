...
test('시작 템플릿 선택과 응답 유실 복구가 같은 생성 요청을 유지한다 @smoke @responsive', async ({ page }) => {
  ...
  await page.goto('/')
  await expect(page.getByRole('combobox', { name: '템플릿 선택', exact: true })).toHaveValue('')
  await page.getByRole('combobox', { name: '템플릿 선택', exact: true }).selectOption('TASK_FORCE_V1')
  await expect(page.getByRole('region', { name: '템플릿 미리보기' })).toContainText('진행 상황과 장애물 확인')
  await page.getByLabel('팀 이름').fill('출시 준비 TF')
  ...
  expect(JSON.parse(pending[0]!.normalizedPayload).template).toBe('TASK_FORCE_V1')

  await page.reload()
  await page.locator('.pending-workspaces summary').click()
  const recovery = page.getByRole('region', { name: '생성 결과 확인 필요' })
  await expect(recovery).toContainText('TF 기본 구성')
  await recovery.getByRole('button', { name: '출시 준비 TF 2026 가을 저장된 입력 불러오기' }).click()
  await expect(page.getByRole('combobox', { name: '템플릿 선택', exact: true })).toHaveValue('TASK_FORCE_V1')
  ...
  expect((attempts[1]!.body as CreateWorkspaceRequest).template).toBe('TASK_FORCE_V1')
