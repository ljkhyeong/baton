...
test('@smoke @responsive 오늘은 업무에 집중하고 필요한 역할과 팀 정보를 열 수 있다', async ({ page }, testInfo) => {
  ...
  const addRoutine = page.getByRole('button', { name: '업무 추가', exact: true })
  await addRoutine.focus()
  await addRoutine.press('Enter')
  const dialog = page.getByRole('dialog', { name: '반복 업무 만들기' })
  await expect(dialog.getByLabel('반복 업무 이름')).toBeFocused()
  await dialog.getByRole('button', { name: '취소' }).click()
  await expect(dialog).toHaveCount(0)
  await expect(addRoutine).toBeFocused()
...
test('@responsive 역할 상세는 desktop 보조 패널과 1100px drawer 경계를 구분한다', async ({ page }, testInfo) => {
  ...
  await expect(reopenedDrawer.getByRole('button', { name: '자료 추가' })).toBeFocused()
