...
for (const access of ['ended', 'viewer'] as const) {
  test(`${access === 'ended' ? '종료 시즌은' : '공유 키 없이 접속한 열람자는'} 저장된 브리프만 조회하고 생성을 막는다 @smoke`, async ({ page }) => {
    ...
    if (access === 'ended') {
      ...
      await expect(panel.getByText('종료된 시즌은 저장된 브리프만 조회할 수 있습니다.')).toHaveCount(0)
    }
