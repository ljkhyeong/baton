...
test('최신 주간 요약 없음과 전달 대기 뒤 생성·재사용·권한 거부를 구분한다 @smoke', async ({ page }, testInfo) => {
  ...
  const generate = edition.getByRole('button', { name: '이번 주 요약 생성', exact: true })
  ...
  await expect(edition.getByRole('region', { name: '이월된 미해결 항목' })).toContainText('handoff:carry')
  expect(await panel.evaluate((element) => element.scrollWidth <= element.clientWidth)).toBe(true)
  await edition.getByRole('region', { name: '이월된 미해결 항목' }).scrollIntoViewIfNeeded()
  await page.screenshot({ path: testInfo.outputPath('brief-edition.png'), fullPage: true })
  await generate.click()
  await expect(edition.getByRole('status')).toContainText('기존 주간 요약을 재사용했습니다.')
  ...
  denied = true
  await edition.getByRole('button', { name: '최신 주간 요약 조회' }).click()
...
for (const access of ['ended', 'viewer'] as const) {
  test(`${access === 'ended' ? '종료 시즌은' : '공유 키 없이 접속한 열람자는'} 저장된 주간 요약만 조회하고 생성을 막는다 @smoke`, async ({ page }) => {
    ...
    await panel.locator('summary').click()
    await panel.getByText('저장된 주간 요약', { exact: true }).click()
    await expect(panel.getByRole('region', { name: '이전 주간 요약 · 분류 미기록' })).toContainText('legacy:+& 한글')
    await expect(panel.getByText('이전 주간 요약: 원본 버전·누락 이력 미기록')).not.toBeVisible()
    await panel.getByText('원본 기록 보기', { exact: true }).click()
    await expect(panel.getByText('이전 주간 요약: 원본 버전·누락 이력 미기록')).toBeVisible()
    if (access === 'ended') {
    ...
    await expect(panel.getByRole('button', { name: '이번 주 요약 생성', exact: true })).toBeDisabled()
  })
}
