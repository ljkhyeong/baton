...
test('서로 다른 탭의 생성 임시 기록을 순서대로 보존하고 응답 유실 뒤 같은 키로 복구한다', async ({ page, context }) => {
  ...
  await fillWorkspaceB()
  await pageB.getByRole('button', { name: '작업 공간 만들기' }).click()
  await expect(pageB.getByRole('heading', { level: 1, name: '남은 업무 0개' })).toBeVisible()
...
test('다른 탭이 생성 결과를 확인하는 동안 온보딩 임시 기록 폐기를 막는다', async ({ page, context }) => {
  ...
  await expect(peerRegion.getByRole('alert')).toContainText('다른 탭에서 생성 결과를 확인하고 있습니다.')
...
test('온보딩 임시 기록 한 건을 확인 후 폐기하고 새 작업 공간을 만든다', async ({ page }) => {
  ...
  await expect(page.getByRole('alert')).toContainText('생성 결과를 확인하지 못한 작업 공간이 5개입니다.')
  expect(api.calls.filter((call) => call.path === '/api/v1/workspaces')).toHaveLength(0)
  ...
  const confirmation = firstItem.getByRole('group', { name: '목록에서 삭제할까요?' })
...
test('@smoke 이 기기에서 삭제는 접근 키와 최근 목록과 ROUND 기록을 함께 지운다', async ({ page, context }) => {
  ...
  await expect(peer.getByRole('heading', { level: 1, name: /남은 업무 \d+개/ })).toBeVisible()
