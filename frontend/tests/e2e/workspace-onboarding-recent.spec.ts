...
test('온보딩 terminal 기록 cleanup이 실패하면 재전송 전에 정리를 요구한다', async ({ page }) => {
  ...
  await expect(page.getByText(
    '브라우저의 임시 요청 기록을 삭제하지 못했습니다. 브라우저 저장을 허용한 뒤 임시 요청 기록 삭제를 다시 시도해 주세요.',
    { exact: true },
  )).toBeVisible()
...
test('서로 다른 탭의 생성 pending을 순서대로 보존하고 응답 유실 뒤 같은 키로 복구한다', async ({ page, context }) => {
  ...
  await fillWorkspaceB()
  await pageB.getByRole('button', { name: '작업 공간 만들기' }).click()
  await expect(pageB.getByRole('heading', { level: 1, name: '이번 회차 미완료 업무 0건' })).toBeVisible()
...
test('@smoke 이 기기 권한 제거는 접근 키와 최근 목록과 ROUND 기록을 함께 지운다', async ({ page, context }) => {
  ...
  await expect(peer.getByRole('heading', { level: 1, name: /이번 회차 미완료 업무/ })).toBeVisible()
