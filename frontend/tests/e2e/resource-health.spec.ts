  ...
  test(`@operations @responsive 자료 연결 상태와 링크를 분리한다: ${mode}`, async ({ page }, testInfo) => {
    ...
    if (mode === 'healthy') {
      await page.route('**/api/v1/auth/session', (route) => route.fulfill({ json: {
        authenticated: true, accountId, csrfHeaderName: 'X-CSRF-TOKEN', csrfToken: 'watch-test-csrf',
      } }))
    }
    ...
    await page.route(`**${SCOPE_PATH}/role-resources/${CREATED_ROLE_RESOURCE_ID}/check-requests`, async (route) => {
      ...
      if (mode === 'healthy') expect(route.request().headers()['x-baton-account-id']).toBe(accountId)
      checks++
