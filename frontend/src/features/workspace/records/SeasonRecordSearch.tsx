...
import { getWorkspace, type WorkspaceScope } from '../api'
...
export function SeasonRecordSearch({ scope, workspace, allSeasons, onAllSeasonsChange, filters, onFiltersChange, onOpenResult }: {
  ...
  const queries = useQueries({ queries: seasons.map(season => ({
    ...
    staleTime: 0,
    retry: (count: number, error: Error) => !(error instanceof ApiError && error.code === 'WORKSPACE_ACCESS_DENIED') && count < 1,
    refetchInterval: (query: { state: { error: Error | null } }) => query.state.error instanceof ApiError && query.state.error.code === 'WORKSPACE_ACCESS_DENIED' ? false : 30_000,
