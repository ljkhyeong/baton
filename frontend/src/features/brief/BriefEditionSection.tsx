...
const sectionNames = { CURRENT_WEEK: '이번 주 변경', CARRY_OVER: '이월된 미해결 항목' }
...
function BriefEditionResults({ scope, timeZone, readOnly, changesDisabled, onOpenSource, onGenerated, navigation, workspaceName }: Props) {
  ...
  return <section aria-label="저장된 주간 요약">
    ...
    <section className="brief-readiness" aria-label="주간 요약 생성 가능 여부">
      <strong>{ready ? readinessText[ready.status] : readiness.isError ? '주간 요약 생성 가능 여부를 확인하지 못했습니다.' : '주간 요약 생성 가능 여부 확인 중…'}</strong>
      {ready && <>
    ...
    <div className="brief-pagination">
      <button type="button" disabled={latest.isFetching || generation.isPending} onClick={refresh}>최신 주간 요약 조회</button>
      <button type="button" disabled={readOnly || changesDisabled || ready?.status !== 'READY' || readiness.isFetching || generation.isPending || shown.isFetching}
        onClick={() => generation.mutate()}>{generation.isPending ? '이번 주 요약 생성 중…' : '이번 주 요약 생성'}</button>
    </div>
    {readOnly ? <p>종료된 시즌은 저장된 주간 요약만 조회할 수 있습니다.</p>
      : <p className="brief-note">시즌 시간대의 이번 주 요약을 생성합니다. 기존 주간 요약을 덮어쓰지 않으며 같은 상태는 재사용합니다.</p>}
    {generation.isError && <p role="alert">{generation.error.message} 전달 상태와 최신 주간 요약을 확인한 뒤 다시 요청해 주세요.</p>}
    {generation.isSuccess && <p role="status">{generation.data.created ? '새 주간 요약을 생성했습니다.' : '기존 주간 요약을 재사용했습니다.'} 생성 번호 {generation.data.generation}</p>}
    <div className="brief-filters">
      <label>조회할 주간 요약<select value={selectedId} onChange={(event) => { navigation.update({ selectedId: event.target.value, baseId: '', previousTargetId: '' }) }}>
        <option value="">최신 주간 요약</option>
        {selectedId && !entries.some((entry) => entry.editionId === selectedId) && <option value={selectedId}>
          {edition ? `${edition.weekStart} 시작 주 · 생성 번호 ${edition.generation}` : '선택한 주간 요약 확인 중'}
        </option>}
      ...
      <label>비교 기준 주간 요약<select value={previousMode ? 'previous-week' : baseId} disabled={!edition} onChange={(event) => { navigation.update({ baseId: event.target.value, previousTargetId: '' }) }}>
    ...
    {previousMode && <div>
      {previous.isPending && <p role="status">선택한 주간 요약의 지난주 마지막 주간 요약을 찾고 있습니다.</p>}
    ...
    {history.isError && <p role="alert">이전 주간 요약 목록을 불러오지 못했습니다. <button type="button" onClick={() => void history.refetch()}>이력 다시 조회</button></p>}
    {history.hasNextPage && <button type="button" disabled={history.isFetching} onClick={() => void history.fetchNextPage()}>이전 주간 요약 더보기</button>}
    ...
    {edition && time && <>
      ...
      <section className="brief-delivery-status" aria-label="저장 이후 변경 확인">
        ...
        {deliveryData?.status === 'ADDITIONAL_DELIVERIES' && <p>{readOnly
          ...
          : '현재 점검 항목을 확인하고, 생성 가능한 상태가 되면 이번 주 요약을 요청할 수 있습니다.'}</p>}
...
function EditionItems({ items, zoneId, readOnly }: { items: BriefEdition['items']; zoneId: string; readOnly: boolean }) {
  ...
  return <ul className="brief-items">{items.map((item) => <li key={`${item.reasonCode}:${item.sourceReference}`}>
    ...
    <details className="brief-evidence"><summary>원본 기록 보기</summary>
      ...
      <small>{item.aggregateRevision === null ? '이전 주간 요약: 원본 버전·누락 이력 미기록'
        : `원본 버전 ${item.aggregateRevision} · ${item.revisionGap ? '누락 이력 있음' : '누락 이력 없음'}`}</small>
    </details>
  </li>)}</ul>
}
