...
const sourceActions: Partial<Record<AttentionCursor['eventType'], { label: string; description: string }>> = {
  ...
  ROLE_PREPARATION_INCOMPLETE: { label: '인수인계 준비 확인', description: '역할의 목적·책임과 필요한 자료를 확인해 주세요.' },
  ROUTINE_REPEATEDLY_OVERDUE: { label: '지연된 업무 확인', description: '업무의 마감과 회차별 진행 상태를 확인해 주세요.' },
...
export function BriefSourceLink({ item, readOnly }: { item: Item & { status: string }; readOnly: boolean }) {
  ...
  </div> : <small>{loading ? '현재 업무 확인 중…' : '관련 업무를 열 수 없습니다.'}</small>
}
